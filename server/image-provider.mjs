...
function publicConfig(config = row()) {
  if (!config) return {
    ...
    modelId: "gpt-image-1", keyHint: null, creditCost: 10, lastTestStatus: null,
...
function normalize(data, existing = row()) {
  const adapter = data.adapter === "minimax" ? "minimax" : "openai";
  ...
  const baseUrl = safeBaseUrl(data.baseUrl || existing?.base_url || defaults.baseUrl);
  ...
  if (!apiKey && !existing) throw imageError("IMAGE_API_KEY_REQUIRED", 422);
  return { adapter, baseUrl, modelId, apiKey, status, creditCost };
}
...
async function imageBytes(payload, fetchImpl) {
  ...
  const source = payload?.data?.[0]?.url || payload?.data?.image_urls?.[0];
  let url;
...
export async function saveImageProviderConfiguration(data, actorUserId = null, fetchImpl = fetch) {
  ...
  const encrypted = input.apiKey ? encryptCredential(apiKey, owner, purpose, version) : {
  ...
  `).run(purpose, input.adapter, input.baseUrl, input.modelId, encrypted.ciphertext, encrypted.iv,
    encrypted.tag, input.apiKey ? keyHint(apiKey) : existing.key_hint, version, input.status,
    input.creditCost, tested.latencyMs, tested.testedAt, actorUserId, existing?.created_at || timestamp, timestamp);
  return publicConfig();
}
