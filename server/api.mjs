...
import {
  addStockAlert,
  addStockWatch,
  markStockAlertTriggered,
  registerStockPetDevice,
  removeStockAlert,
  removeStockPetDevice,
  removeStockWatch,
  stockPetDownload,
  stockPetLicense,
...
async function optimizedImage(buffer, { width = 256, quality = 80 } = {}) {
  return sharp(buffer, { failOn: "none" })
    ...
    .webp({ quality, alphaQuality: 86, effort: 4 })
    .toBuffer();
}
...
const phoneOnlyEmail = (email) =>
  String(email || "").endsWith("@phone.oneshowtools.invalid");
const wechatOnlyEmail = (email) =>
  String(email || "").endsWith("@wechat.oneshowtools.invalid");
const cleanUser = (row) => {
  if (!row) return row;
  const phone = db
    .prepare(
      "SELECT phone_last4 AS last4, country_code AS countryCode FROM user_phone_identities WHERE user_id = ?",
    )
    .get(row.id);
  const wechat = db
    .prepare(
      "SELECT 1 AS linked FROM provider_accounts WHERE user_id = ? AND provider = 'wechat_miniprogram'",
    )
    .get(row.id);
  const syntheticEmail =
    phoneOnlyEmail(row.email) || wechatOnlyEmail(row.email);
  return {
    id: row.id,
    name: row.name,
...
function rateLimited(request, action, subject, maximum = 5, windowMs = 300000) {
  ...
  const key = hashToken(
    `${action}:${String(subject || "").toLowerCase()}:${ipHash}`,
  );
...
function accountVerified(user) {
  return Boolean(
    ...
    db
      .prepare(
        "SELECT 1 AS verified FROM user_phone_identities WHERE user_id = ?",
      )
      .get(user?.id),
  );
}
...
async function handleAdmin(request, path) {
  ...
  if (path === "/api/admin/users" && request.method === "GET") {
    ...
    `,
      ...
      .all(...params)
      .map((user) => ({ ...user, emailVerified: Boolean(user.emailVerified) }));
    return json({ users });
  }
  ...
  if (statusMatch && request.method === "PATCH") {
    ...
    if (targetId === admin.id && status === "suspended")
      return fail("CANNOT_SUSPEND_SELF", 409);
    const result = db
...
async function register(request) {
  ...
  if (
    ...
    data.termsVersion !== "2026-08-24" ||
    data.privacyVersion !== "2026-08-24"
  )
    return fail("LEGAL_CONSENT_REQUIRED");
  ...
  if (!user) {
    ...
    db.prepare(
      "INSERT OR IGNORE INTO policy_acceptances (id, user_id, policy_type, policy_version, source, accepted_at) VALUES (?, ?, ?, ?, 'email_registration', ?)",
    ).run(randomUUID(), id, "terms", data.termsVersion, timestamp);
    db.prepare(
      "INSERT OR IGNORE INTO policy_acceptances (id, user_id, policy_type, policy_version, source, accepted_at) VALUES (?, ?, ?, ?, 'email_registration', ?)",
    ).run(randomUUID(), id, "privacy", data.privacyVersion, timestamp);
  ...
  return json(
    {
      user: cleanUser(user),
      ...(clientKind
        ? { accessToken: token, expiresAt: timestamp + 14 * 86400000 }
        : {}),
    },
    status,
...
async function login(request) {
  ...
  if (
    !user ||
    !(await verifyPassword(String(data.password || ""), user.password_hash))
  ) {
...
function smsLimits(phoneHash) {
  ...
  if (latest && timestamp - latest.createdAt < 60000)
    return {
      limited: true,
      retryAfter: Math.ceil((60000 - (timestamp - latest.createdAt)) / 1000),
    };
  if (hour >= 5 || day >= 10)
    return { limited: true, retryAfter: hour >= 5 ? 3600 : 86400 };
...
async function sendSmsVerification(request) {
  ...
  let phone;
  try {
    phone = normalizeMainlandPhone(data.phone);
  } catch (error) {
    return fail(error.code || "INVALID_PHONE", error.status || 400);
  }
  const phoneHash = phoneIdentityHash(phone);
  const limit = smsLimits(phoneHash);
  if (
    limit.limited ||
    rateLimited(request, "sms-send", phoneHash, 10, 3600000)
  ) {
    securityEvent(request, "auth.sms.send", "rate_limited", null, {
    ...
    return json(
      {
        error: {
          ...
          retryAfter: limit.retryAfter || 3600,
        },
      },
      429,
    );
  ...
  try {
    ...
    securityEvent(request, "auth.sms.send", "provider_failed", null, {
  ...
  return json(
    {
      ok: true,
      expiresIn: Number(process.env.SMS_CODE_TTL_SECONDS || 300),
      retryAfter: 60,
    },
    202,
  );
...
async function verifySmsLogin(request) {
  ...
  const phoneHash = phoneIdentityHash(phone);
  if (rateLimited(request, "sms-verify", phoneHash, 10, 600000))
  ...
  if (
    ...
    securityEvent(
      request,
      "auth.sms.verify",
      verification?.expires_at <= Date.now() ? "expired" : "denied",
    );
    return fail(
      verification?.expires_at <= Date.now()
        ? "SMS_CODE_EXPIRED"
  ...
  if (!userId) {
    if (
      ...
      return fail("LEGAL_CONSENT_REQUIRED");
    const name =
      data.locale === "en" ? `User_${phone.last4}` : `用户_${phone.last4}`;
    userId = randomUUID();
    ...
    try {
      ...
      db.prepare(
        "INSERT OR IGNORE INTO policy_acceptances (id, user_id, policy_type, policy_version, source, accepted_at) VALUES (?, ?, ?, ?, 'sms_registration', ?)",
      ).run(randomUUID(), userId, "terms", data.termsVersion, timestamp);
      db.prepare(
        "INSERT OR IGNORE INTO policy_acceptances (id, user_id, policy_type, policy_version, source, accepted_at) VALUES (?, ?, ?, ?, 'sms_registration', ?)",
      ).run(randomUUID(), userId, "privacy", data.privacyVersion, timestamp);
      db.prepare(
        "UPDATE sms_verification_codes SET consumed_at = ? WHERE id = ? AND consumed_at IS NULL",
      ).run(timestamp, verification.id);
      db.exec("COMMIT");
      ...
      const raced = db
        .prepare(
          "SELECT user_id FROM user_phone_identities WHERE phone_hash = ?",
        )
        .get(phoneHash);
...
async function verifyWechatMiniProgramLogin(request) {
  const config = getServerConfig(request.url);
  if (!config.wechatMiniProgramEnabled)
    return fail("WECHAT_MINIPROGRAM_UNAVAILABLE", 503);
  const data = await body(request);
  const code = String(data.code || "").trim();
  if (!/^[A-Za-z0-9_-]{4,160}$/.test(code))
    return fail("INVALID_WECHAT_CODE", 400);
  if (rateLimited(request, "wechat-miniprogram-login", code, 8, 60000))
    return fail("WECHAT_LOGIN_FAILED", 401);
  ...
  if (!userId) {
    ...
    try {
      ...
      const raced = db
        .prepare(
          `SELECT user_id AS userId FROM provider_accounts
        WHERE provider = 'wechat_miniprogram' AND provider_account_id = ?`,
        )
        .get(providerId);
      if (!raced) throw error;
      userId = raced.userId;
...
async function verifyEmail(request) {
  ...
  if (!token)
    return new Response(null, {
      status: 302,
      headers: { location: `${config.appUrl}/?auth=verification-invalid` },
    });
  const timestamp = Date.now();
  db.exec("BEGIN IMMEDIATE");
  try {
  ...
  return new Response(null, {
    status: 302,
    headers: { location: `${config.appUrl}/?auth=verified` },
  });
}
...
async function resendVerification(request) {
  ...
  const email = String(data.email || "")
    .trim()
    .toLowerCase();
  if (rateLimited(request, "verify-resend", email))
  ...
  if (
    user &&
    !user.email_verified &&
    getServerConfig(request.url).registrationEnabled
  ) {
    await issueAccountToken(request, user, "verify");
  }
...
async function requestPasswordReset(request) {
  ...
  const email = String(data.email || "")
    .trim()
    .toLowerCase();
  if (rateLimited(request, "password-reset-request", email))
    return json({ ok: true }, 202);
  const user = db
    .prepare(
...
async function uploadFile(request, user) {
  ...
  try {
    ...
    return fail(error.code || "USER_FILE_LIMIT_REACHED", error.status || 409);
  }
...
function removeFileRecord(userId, fileId) {
  ...
  if (result.changes)
    db.prepare(
      "DELETE FROM user_favorites WHERE user_id = ? AND item_type = 'file' AND item_id = ?",
    ).run(userId, fileId);
  return result;
}
  ...
  try {
    ...
    `,
      ...
      JSON.stringify({
        ...
        modelConnectionId: toolModelCapability(tool.runtimeKind)
          .userConfigurable
  ...
  return json(
    {
      task: db
        .prepare(
          "SELECT id, status, credit_cost AS creditCost, created_at AS createdAt FROM tasks WHERE id = ?",
        )
        .get(id),
    },
    201,
...
function softDeleteTask(user, id) {
  ...
  if (activeTaskStatuses.has(task.status))
    return { error: "TASK_DELETE_ACTIVE", status: 409 };
  const timestamp = Date.now();
  db.exec("BEGIN IMMEDIATE");
  try {
    db.prepare(
    ...
    db.prepare(
      "DELETE FROM user_favorites WHERE user_id = ? AND item_type = 'prompt' AND item_id = ?",
    ).run(user.id, id);
    db.prepare(
      "DELETE FROM project_items WHERE user_id = ? AND item_type = 'task' AND item_id = ?",
    ).run(user.id, id);
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
  ...
  audit(user.id, "task.delete", "task", id, {
    mode: "soft_delete",
...
function listFavorites(userId) {
  ...
  `,
    ...
    .all(userId)
    .map((item) => ({ ...item, itemCount: Number(item.itemCount || 0) }));
...
function favoriteTargetExists(userId, itemType, itemId) {
  if (itemType === "tool")
    return Boolean(
      db
        .prepare("SELECT id FROM tools WHERE id = ? AND active = 1")
        .get(itemId),
    );
  if (itemType === "prompt")
    return Boolean(
  ...
  if (itemType === "file" || itemType === "material")
    return Boolean(
      db
        .prepare("SELECT id FROM files WHERE id = ? AND user_id = ?")
        .get(itemId, userId),
...
function favoriteCollectionExists(userId, collectionId) {
  return (
    ...
    Boolean(
      db
        .prepare(
          "SELECT id FROM favorite_collections WHERE id = ? AND user_id = ?",
        )
...
async function updateProfile(request, user) {
  ...
  const name = String(data.name || "").trim();
  const locale =
    data.locale === "en" ? "en" : data.locale === "zh-CN" ? "zh-CN" : null;
  if (!name || name.length > 80 || !locale) return fail("INVALID_PROFILE");
  db.prepare(
    "UPDATE users SET name = ?, locale = ?, updated_at = ? WHERE id = ?",
...
async function confirmEmailChange(request) {
  ...
  if (!token)
    return new Response(null, {
      status: 302,
      headers: { location: `${config.appUrl}/?auth=email-change-invalid` },
    });
  const timestamp = Date.now();
  db.exec("BEGIN IMMEDIATE");
  try {
    const conflict = db
      .prepare("SELECT id FROM users WHERE email = ? AND id <> ?")
      .get(token.email, token.user_id);
    if (conflict)
      throw Object.assign(new Error("EMAIL_UNAVAILABLE"), { status: 409 });
    const consumed = db
      .prepare(
        "UPDATE auth_tokens SET consumed_at = ? WHERE id = ? AND consumed_at IS NULL",
      )
  ...
  return new Response(null, {
    status: 302,
    headers: { location: `${config.appUrl}/?auth=email-changed` },
  });
}
...
function revokeSessions(request, user, target) {
  ...
  securityEvent(request, "account.sessions.revoke", "success", user.id, {
    target: target === "others" ? "others" : "selected",
  });
  return json({ ok: true });
}
...
function createExport(user) {
  ...
  const payload = {
    ...
    tasks: db
      .prepare(
        "SELECT id, tool_id AS toolId, status, credit_cost AS creditCost, created_at AS createdAt FROM tasks WHERE user_id = ? AND deleted_at IS NULL ORDER BY created_at",
      )
      .all(user.id),
    files: db
      .prepare(
        "SELECT id, name, mime_type AS mimeType, size_bytes AS sizeBytes, created_at AS createdAt FROM files WHERE user_id = ? ORDER BY created_at",
      )
      .all(user.id),
  };
  db.prepare(
  ...
  return json(
    { export: { id, status: "completed", expiresAt: Date.now() + 86400000 } },
    201,
  );
}
...
function accountPreferences(userId) {
  const row = db
    .prepare(
      "SELECT preferences_json AS preferencesJson FROM user_preferences WHERE user_id = ?",
    )
    .get(userId);
  try {
    return {
      ...preferenceDefaults,
      ...(row ? JSON.parse(row.preferencesJson) : {}),
    };
  } catch {
...
async function updateAccountPreferences(request, user) {
  ...
  const next = {
    timezone: ["Asia/Shanghai", "UTC", "America/Los_Angeles"].includes(
      input.timezone,
    )
      ? input.timezone
      : preferenceDefaults.timezone,
    dateFormat: ["YYYY-MM-DD", "DD/MM/YYYY", "MM/DD/YYYY"].includes(
      input.dateFormat,
    )
      ? input.dateFormat
      : preferenceDefaults.dateFormat,
    pageSize: ["10", "20", "50"].includes(String(input.pageSize))
      ? String(input.pageSize)
      : preferenceDefaults.pageSize,
...
async function createWorkspaceProject(request, user) {
  ...
  const now = Date.now();
  db.prepare(
    "INSERT INTO workspace_projects (id,user_id,name,description,status,created_at,updated_at) VALUES (?,?,?,?, 'active',?,?)",
  ).run(
    id,
    user.id,
    name,
  ...
  return json(
    {
      project: projectPayload(
        db
          .prepare(
            "SELECT id,name,description,status,created_at AS createdAt,updated_at AS updatedAt FROM workspace_projects WHERE id = ?",
          )
          .get(id),
      ),
    },
...
async function updateWorkspaceProject(request, user, id) {
  const existing = db
    .prepare("SELECT * FROM workspace_projects WHERE id = ? AND user_id = ?")
    .get(id, user.id);
  ...
  const name =
    input.name === undefined
      ...
      : String(input.name || "")
          ...
          .slice(0, 80);
  const status =
    input.status === undefined ? existing.status : String(input.status);
  if (!name) return fail("PROJECT_NAME_REQUIRED", 400);
  if (!["active", "completed", "archived"].includes(status))
    return fail("PROJECT_STATUS_INVALID", 400);
  db.prepare(
    "UPDATE workspace_projects SET name=?,description=?,status=?,updated_at=? WHERE id=? AND user_id=?",
  ).run(
    name,
  ...
  return json({
    project: listWorkspaceProjects(user.id).find((item) => item.id === id),
  });
}

function deleteWorkspaceProject(user, id) {
  const result = db
    .prepare("DELETE FROM workspace_projects WHERE id = ? AND user_id = ?")
    .run(id, user.id);
...
async function requestDeletion(request, user) {
  ...
  const data = await body(request);
  if (!(await verifyPassword(String(data.password || ""), user.password_hash)))
    return fail("INVALID_CREDENTIALS", 401);
  const id = randomUUID();
  const timestamp = Date.now();
  db.prepare(
...
async function billingCheckout(request, user) {
  ...
  if (provider !== "stripe" || !config.billingEnabled)
  ...
  const metadata = {
    userId: user.id,
    planId: plan.id,
    credits: String(offer.totalCredits),
  };
...
function reconcileStripeEvent(event) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (object.mode === "subscription" && object.subscription) {
      ...
      if (
        !db
          .prepare("SELECT id FROM plans WHERE id = ? AND interval = 'month'")
          .get(planId)
      )
        return;
      db.prepare(
  ...
  if (event.type.startsWith("customer.subscription.")) {
    ...
    const existing = db
      .prepare(
        "SELECT plan_id FROM subscriptions WHERE provider = 'stripe' AND provider_subscription_id = ?",
      )
  ...
  if (
    ...
    if (event.type === "invoice.paid") {
      ...
      `,
        ...
        randomUUID(),
        user.id,
        subscription.recurringCredits,
        `${subscription.nameZh}月度积分`,
        `${subscription.nameEn} monthly credits`,
        object.id,
        timestamp,
      );
    }
...
const handleAdminV1 = createAdminHandler({
  currentUser,
  currentSession,
  issueAccountToken,
});
...
export async function handleApi(request) {
  ...
  if (
    path === "/api/internal/v1/platform-model/invoke" &&
    request.method === "POST"
  ) {
    if (!internalServiceAuthorized(request))
      return fail("SERVICE_UNAUTHENTICATED", 401);
    try {
      const data = await body(request);
      if (data.purpose !== "oneshow_home_chat")
        return fail("INVALID_PLATFORM_MODEL_PURPOSE", 400);
      ...
      return fail(
        error.code || "PLATFORM_MODEL_INVOCATION_FAILED",
        error.status || 502,
      );
    }
  }
  if (path === "/api/health" && request.method === "GET")
    return json({
      ...
      billingEnabled:
        config.billingEnabled || activePaymentProviders().length > 0,
      ...
      configurationErrors,
      oneShowModelEnabled:
        gatewayFlags().managedConfigured &&
        gatewayFlags().managedExecutionEnabled,
  ...
  if (path === "/api/billing/webhooks/alipay" && request.method === "POST") {
    try {
      await handleAlipayNotification(request);
      return new Response("success", {
        status: 200,
        headers: { "content-type": "text/plain; charset=utf-8" },
      });
    } catch (error) {
      securityEvent(request, "billing.alipay.webhook", error.code || "failed");
      return new Response("failure", { status: error.status || 400 });
    }
  }
  if (path === "/api/billing/webhooks/wechat" && request.method === "POST") {
    try {
      await handleWechatNotification(request);
      return json({ code: "SUCCESS", message: "成功" });
    } catch (error) {
      securityEvent(request, "billing.wechat.webhook", error.code || "failed");
      return json(
        { code: "FAIL", message: error.code || "支付通知处理失败" },
        error.status || 400,
      );
    }
  }
  const singingCallbackMatch = path.match(
    /^\/api\/music\/singing-provider\/callback\/([^/]+)$/,
  );
  if (singingCallbackMatch && request.method === "POST") {
    try {
      return json(
        await handleSingingProviderCallback(request, singingCallbackMatch[1]),
      );
    } catch (error) {
      return fail(error.code || "SINGING_CALLBACK_FAILED", error.status || 500);
    }
  }
  if (path === "/api/auth/verify" && request.method === "GET")
    return verifyEmail(request);
  if (path === "/api/auth/confirm-email" && request.method === "GET")
    return confirmEmailChange(request);
  if (!sameOrigin(request, config.appUrl))
  ...
  if (path === "/api/auth/sms/verify" && request.method === "POST")
    return verifySmsLogin(request);
  if (path === "/api/auth/wechat-miniprogram" && request.method === "POST")
    return verifyWechatMiniProgramLogin(request);
  ...
  if (publicToolIconMatch && request.method === "GET") {
    let slug;
    try {
      slug = decodeURIComponent(publicToolIconMatch[1]);
    } catch {
      return fail("INVALID_TOOL_SLUG");
    }
    const branding = db
      .prepare(
        `
    ...
    try {
      ...
      if (!stored)
        stored = cacheImage(
          cacheKey,
          await optimizedImage(await readStoredFile(branding)),
        );
  ...
  if (path === "/api/plans" && request.method === "GET") {
    const plans = db
      .prepare("SELECT * FROM plans WHERE active = 1")
      .all()
      .map(billingPlanPayload)
  ...
  if (path === "/api/products/stock-pet/unlock" && request.method === "POST") {
    try {
      return json(unlockStockPet(user.id), 201);
    } catch (error) {
      return fail(error.code || "PRODUCT_UNLOCK_FAILED", error.status || 500);
    }
  }
  if (path === "/api/products/stock-pet/devices" && request.method === "POST") {
    try {
      return json(registerStockPetDevice(user.id, await body(request)), 201);
    } catch (error) {
      return fail(
        error.code || "DEVICE_REGISTRATION_FAILED",
        error.status || 500,
      );
    }
  }
  const stockPetDeviceMatch = path.match(
    /^\/api\/products\/stock-pet\/devices\/([^/]+)$/,
  );
  if (stockPetDeviceMatch && request.method === "DELETE")
    return json(removeStockPetDevice(user.id, stockPetDeviceMatch[1]));
  if (
    path.startsWith("/api/products/stock-pet/") &&
    request.headers.get("x-oneshow-client") === "desktop"
  ) {
    try {
      verifyStockPetDevice(
        user.id,
        request.headers.get("x-stock-pet-device"),
      );
    } catch (error) {
      return fail(error.code || "DEVICE_NOT_REGISTERED", error.status || 403);
    }
  }
  if (path === "/api/products/stock-pet/watchlist" && request.method === "GET")
    return json({ items: stockWatchlist(user.id) });
  if (
    path === "/api/products/stock-pet/watchlist" &&
    request.method === "POST"
  ) {
    try {
      return json({ items: addStockWatch(user.id, await body(request)) }, 201);
    } catch (error) {
      return fail(error.code || "WATCHLIST_UPDATE_FAILED", error.status || 500);
    }
  }
  if (path === "/api/products/stock-pet/watchlist" && request.method === "PATCH") {
    try {
      return json({ items: updateStockWatchlist(user.id, await body(request)) });
    } catch (error) {
      return fail(error.code || "WATCHLIST_UPDATE_FAILED", error.status || 500);
    }
  }
  const stockWatchMatch = path.match(
    /^\/api\/products\/stock-pet\/watchlist\/([^/]+)$/,
  );
  if (stockWatchMatch && request.method === "DELETE")
    return json({ items: removeStockWatch(user.id, stockWatchMatch[1]) });
  if (path === "/api/products/stock-pet/alerts" && request.method === "GET")
    return json({ items: stockAlerts(user.id) });
  if (path === "/api/products/stock-pet/alerts" && request.method === "POST") {
    try {
      return json({ items: addStockAlert(user.id, await body(request)) }, 201);
    } catch (error) {
      return fail(
        error.code || "STOCK_ALERT_UPDATE_FAILED",
        error.status || 500,
      );
    }
  }
  const stockAlertTriggerMatch = path.match(
    /^\/api\/products\/stock-pet\/alerts\/([^/]+)\/trigger$/,
  );
  if (stockAlertTriggerMatch && request.method === "POST") {
    try {
      return json(markStockAlertTriggered(user.id, stockAlertTriggerMatch[1]));
    } catch (error) {
      return fail(
        error.code || "STOCK_ALERT_UPDATE_FAILED",
        error.status || 500,
  ...
  const stockAlertMatch = path.match(
    /^\/api\/products\/stock-pet\/alerts\/([^/]+)$/,
  );
  if (stockAlertMatch && request.method === "PATCH") {
    try {
      return json({
        items: updateStockAlert(user.id, stockAlertMatch[1], await body(request)),
      });
    } catch (error) {
      return fail(error.code || "STOCK_ALERT_UPDATE_FAILED", error.status || 500);
    }
  }
  if (stockAlertMatch && request.method === "DELETE")
    return json({ items: removeStockAlert(user.id, stockAlertMatch[1]) });
  if (
    path === "/api/products/stock-pet/stocks/search" &&
    request.method === "GET"
  )
    return json({ items: await stockSearch(url.searchParams.get("q")) });
  if (path === "/api/products/stock-pet/quotes" && request.method === "GET") {
    try {
      return json({
        quotes: await stockQuotes(
          String(url.searchParams.get("symbols") || "")
            .split(",")
            .filter((symbol) => stockWatchlist(user.id).some((item) => item.symbol === symbol)),
        ),
      });
    } catch (error) {
  ...
  if (path === "/api/music/tracks" && request.method === "GET")
    return json({ tracks: listMusicTracks(user.id) });
  ...
  if (path === "/api/music/singing-voices" && request.method === "POST") {
    ...
    try {
      return json(
        { voice: await enrollSingingVoice(user, await request.formData()) },
        201,
      );
    } catch (error) {
      return fail(
        error.code || "SINGING_VOICE_ENROLL_FAILED",
        error.status || 500,
      );
    }
  }
  const singingVoiceMatch = path.match(
    /^\/api\/music\/singing-voices\/([^/]+)$/,
  );
  if (singingVoiceMatch && request.method === "DELETE") {
    if (!musicStudioStatus().singingCover.available)
      return fail("FEATURE_NOT_AVAILABLE", 404);
    try {
      return json(await removeSingingVoice(user, singingVoiceMatch[1]));
    } catch (error) {
      return fail(
        error.code || "SINGING_VOICE_DELETE_FAILED",
        error.status || 500,
      );
    }
  }
  if (path === "/api/music/singing-covers" && request.method === "POST") {
    ...
    try {
      return json(
        await submitSingingCover(user, await request.formData()),
        201,
      );
    } catch (error) {
      return fail(
        error.code || "SINGING_COVER_SUBMIT_FAILED",
        error.status || 500,
      );
    }
  }
  if (path === "/api/music/references" && request.method === "POST") {
    ...
    try {
      const form = await request.formData();
      return json(
        { reference: await createMusicReference(user, form.get("file")) },
        201,
      );
    } catch (error) {
      return fail(
        error.code || "MUSIC_REFERENCE_PREPROCESS_FAILED",
        error.status || 500,
      );
    }
  }
  ...
  if (musicTrackMatch && request.method === "DELETE") {
    try {
      return json(await deleteMusicTrack(user.id, musicTrackMatch[1]));
    } catch (error) {
      return fail(
        error.code || "MUSIC_TRACK_DELETE_FAILED",
        error.status || 500,
      );
    }
  }
  ...
  if (musicCoverMatch && request.method === "POST") {
    ...
    try {
      ...
      return fail(
        error.code || "MUSIC_COVER_GENERATION_FAILED",
        error.status || 500,
      );
    }
  }
  ...
  if (path === "/api/dashboard" && request.method === "GET")
    return json(dashboard(user.id));
  if (path === "/api/support/conversations" && request.method === "GET") {
    return json({ conversations: listUserSupportConversations(user.id) });
  }
  const supportConversationMatch = path.match(
    /^\/api\/support\/conversations\/([^/]+)$/,
  );
  if (supportConversationMatch && request.method === "GET") {
    const conversation = getUserSupportConversation(
      user.id,
      supportConversationMatch[1],
    );
    return conversation
      ? json({ conversation })
      : fail("SUPPORT_CONVERSATION_NOT_FOUND", 404);
  }
  ...
  const supportTicketMatch = path.match(
    /^\/api\/support\/conversations\/([^/]+)\/(?:ticket|handoff)$/,
  );
  if (supportTicketMatch && request.method === "POST") {
    const data = await body(request);
    try {
      return json({
        conversation: submitSupportTicket(
          user.id,
          supportTicketMatch[1],
          data.message,
        ),
      });
    } catch (error) {
      return fail(error.code || "SUPPORT_TICKET_FAILED", error.status || 400);
    }
  }
  if (path === "/api/marketplace/search-events" && request.method === "POST") {
    const data = await body(request);
    recordMarketplaceSearch({
      opaqueUserId: hashToken(`market:${user.id}`),
      query: data.query,
  ...
  if (
    path === "/api/marketplace/behavior-events" &&
    request.method === "POST"
  ) {
    const data = await body(request);
    ...
    return recorded
      ? json({ ok: true }, 202)
      : fail("INVALID_MARKETPLACE_EVENT");
  }
  if (path === "/api/account/profile" && request.method === "PATCH")
    return updateProfile(request, user);
  if (path === "/api/account/preferences" && request.method === "GET")
    return json({ preferences: accountPreferences(user.id) });
  if (path === "/api/account/preferences" && request.method === "PATCH")
    return updateAccountPreferences(request, user);
  ...
  if (path === "/api/account/email" && request.method === "POST")
    return requestEmailChange(request, user);
  if (path === "/api/account/sessions" && request.method === "GET")
    return listSessions(request, user);
  if (path === "/api/account/sessions/others" && request.method === "DELETE")
    return revokeSessions(request, user, "others");
  if (
    path.match(/^\/api\/account\/sessions\/[^/]+$/) &&
    request.method === "DELETE"
  ) {
    return revokeSessions(request, user, path.split("/")[4]);
  }
  if (path === "/api/account/export" && request.method === "POST")
    return createExport(user);
  if (
    path.match(/^\/api\/account\/exports\/[^/]+\/download$/) &&
    request.method === "GET"
  ) {
  ...
  if (path === "/api/account/deletion" && request.method === "POST")
    return requestDeletion(request, user);
  ...
  if (path === "/api/projects" && request.method === "GET")
    return json({ projects: listWorkspaceProjects(user.id) });
  if (path === "/api/projects" && request.method === "POST")
    return createWorkspaceProject(request, user);
  const workspaceProjectMatch = path.match(/^\/api\/projects\/([^/]+)$/);
  if (workspaceProjectMatch && request.method === "PATCH")
    return updateWorkspaceProject(request, user, workspaceProjectMatch[1]);
  if (workspaceProjectMatch && request.method === "DELETE")
    return deleteWorkspaceProject(user, workspaceProjectMatch[1]);
  ...
  if (path === "/api/favorites" && request.method === "POST") {
    ...
    if (!favoriteTargetExists(user.id, itemType, itemId))
      return fail("FAVORITE_TARGET_NOT_FOUND", 404);
    if (!favoriteCollectionExists(user.id, collectionId))
      return fail("FAVORITE_COLLECTION_NOT_FOUND", 404);
    const timestamp = Date.now();
    const existing = db
      .prepare(
    ...
    if (existing) {
      db.prepare(
        "UPDATE user_favorites SET collection_id = ?, updated_at = ? WHERE id = ? AND user_id = ?",
      ).run(collectionId, timestamp, existing.id, user.id);
    } else {
      db.prepare(
        "INSERT INTO user_favorites (id,user_id,item_type,item_id,collection_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
      ).run(
        randomUUID(),
        user.id,
        itemType,
  ...
  if (favoriteMatch && request.method === "DELETE") {
    const favorite = db
      .prepare(
        "SELECT item_type AS itemType, item_id AS itemId FROM user_favorites WHERE id = ? AND user_id = ?",
      )
      .get(favoriteMatch[1], user.id);
    if (!favorite) return fail("FAVORITE_NOT_FOUND", 404);
    db.prepare("DELETE FROM user_favorites WHERE id = ? AND user_id = ?").run(
      favoriteMatch[1],
      user.id,
    );
  ...
  if (path === "/api/favorite-collections" && request.method === "POST") {
    ...
    try {
      db.prepare(
        "INSERT INTO favorite_collections (id,user_id,name,created_at,updated_at) VALUES (?,?,?,?,?)",
      ).run(randomUUID(), user.id, name, timestamp, timestamp);
    } catch {
      return fail("FAVORITE_COLLECTION_EXISTS", 409);
    }
    audit(user.id, "favorite_collection.create", "user", user.id, { name });
    return json(listFavorites(user.id), 201);
  }
  const favoriteCollectionMatch = path.match(
  ...
  if (favoriteCollectionMatch && request.method === "DELETE") {
    const result = db
      ...
      .run(favoriteCollectionMatch[1], user.id);
  ...
  if (path === "/api/model-connections/validate" && request.method === "POST") {
    ...
    try {
      ...
      audit(user.id, "model.connection.validate", "model_connection", null, {
  ...
  if (connectionMatch && request.method === "PATCH") {
    try {
      const connection = updateModelConnection(
        user.id,
        connectionMatch[1],
        await body(request),
      );
  ...
  const connectionRotate = path.match(
    /^\/api\/model-connections\/([^/]+)\/rotate$/,
  );
  if (connectionRotate && request.method === "POST") {
    try {
      const data = await body(request);
      const connection = rotateModelCredential(
        user.id,
        connectionRotate[1],
        data.apiKey,
      );
  ...
  const connectionTest = path.match(
    /^\/api\/model-connections\/([^/]+)\/test$/,
  );
  ...
  if (path.match(/^\/api\/tool-actions\/[^/]+$/) && request.method === "POST") {
    ...
    try {
      ...
      return fail(
        error.code || error.message || "TOOL_ACTION_FAILED",
        error.status || 500,
      );
    }
  }
  if (path === "/api/tasks/bulk-delete" && request.method === "POST") {
    const data = await body(request);
    const ids = [
      ...new Set((Array.isArray(data.ids) ? data.ids : []).map(String)),
    ].slice(0, 100);
  ...
  if (taskDetailMatch && request.method === "GET") {
    const task = listTasks(user.id).find(
      (item) => item.id === taskDetailMatch[1],
    );
  ...
  if (toolModelMatch && request.method === "PATCH") {
    try {
      const data = await body(request);
      const preference = setToolModelPreference(
        user.id,
        toolModelMatch[1],
      ...
      audit(user.id, "tool.model.update", "tool", toolModelMatch[1], {
        route:
          preference.modelConnectionId === "managed"
            ? "managed"
            : "user_connection",
      });
      ...
      return fail(
        error.code || "TOOL_MODEL_UPDATE_FAILED",
        error.status || 500,
      );
    }
  }
  if (
    path.match(/^\/api\/tasks\/[^/]+\/cancel$/) &&
    request.method === "POST"
  ) {
  ...
  if (path === "/api/files" && request.method === "GET") {
    ...
    const limit = Math.min(
      ...
      Math.max(
        1,
        Number.parseInt(requestUrl.searchParams.get("limit") || "100", 10) ||
          100,
      ),
    );
  ...
  if (path === "/api/files/bulk-delete" && request.method === "POST") {
    ...
    const files = db
      .prepare(
        `SELECT f.*, COALESCE(s.provider, 'local') AS storage_provider, s.object_key
      FROM files f LEFT JOIN file_storage_objects s ON s.file_id = f.id
  ...
  if (
    ...
    const file = db
      .prepare(
        `SELECT f.*, COALESCE(s.provider, 'local') AS storage_provider, s.object_key
      FROM files f LEFT JOIN file_storage_objects s ON s.file_id = f.id WHERE f.id = ? AND f.user_id = ?`,
      )
      .get(id, user.id);
  ...
  if (
    path.match(/^\/api\/files\/[^/]+\/thumbnail$/) &&
    request.method === "GET"
  ) {
    const id = path.split("/")[3];
    const file = db
      .prepare(
        `SELECT f.*, COALESCE(s.provider, 'local') AS storage_provider, s.object_key, s.etag
      FROM files f LEFT JOIN file_storage_objects s ON s.file_id = f.id WHERE f.id = ? AND f.user_id = ?`,
      )
      .get(id, user.id);
    if (!file) return fail("FILE_NOT_FOUND", 404);
    if (!String(file.mime_type || "").startsWith("image/"))
      return fail("FILE_THUMBNAIL_UNSUPPORTED", 415);
    ...
    try {
      ...
      return fail("FILE_THUMBNAIL_FAILED", 422);
    }
  }
  ...
  if (path === "/api/billing/checkout" && request.method === "POST")
    return billingCheckout(request, user);
  ...
  if (billingOrderMatch && request.method === "GET") {
    try {
      ...
      return fail(error.code || "PAYMENT_ORDER_NOT_FOUND", error.status || 404);
    }
  }
