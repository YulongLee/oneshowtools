...
function OperationsView({ data, locale, onRetry }) {
  ...
  return <section className="admin-v2-panel admin-table-panel"><nav className="admin-section-tabs">{Object.keys(sections).map((key) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{t[key]} <span>{sections[key]?.length || 0}</span></button>)}</nav>
    <div className="admin-v2-table-wrap"><table><thead><tr><th>{t.action}</th><th>{t.status}</th><th>{t.target}</th><th>Correlation ID</th><th>{t.time}</th><th /></tr></thead><tbody>
      ...
      {!sections[tab]?.length && <tr><td colSpan="6" className="admin-empty">{t.noData}</td></tr>}</tbody></table></div></section>;
}
