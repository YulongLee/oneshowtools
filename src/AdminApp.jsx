...
import {
  ...
  Copy as CopyIcon, DownloadSimple, Plus, QrCode, Target, ArrowRight, Eye,
...
const copy = {
  "zh-CN": {
    ...
    model_studio_workspace: "百炼工作空间", stock_market_data: "股票行情数据", music_generation: "OneShowMusic", singing_cover: "歌曲翻唱模型", image_generation: "图片生成模型", image_editing: "图片编辑模型", image_upscaling: "高清修复模型", image_text_ocr: "图片文字识别", musicProvider: "音乐模型", musicProviderTitle: "OneShowMusic 生成服务", musicProviderHint: "管理 AI 音乐工作室使用的服务端音乐模型。密钥只在后端加密保存，不会发送到浏览器。",
    ...
    managed_runtime: "OneShowModel", market_intelligence: "市场情报模型", oneshow_home_chat: "OneShow Home 对话", food_nutrition: "食物营养识别模型", modelName: "配置名称", modelProtocol: "接口协议",
  ...
  en: {
    ...
    model_studio_workspace: "Model Studio Workspace", stock_market_data: "Stock Market Data", music_generation: "OneShowMusic", singing_cover: "Song Cover Model", image_generation: "Image Generation Model", image_editing: "Image Editing Model", image_upscaling: "Image Restoration Model", image_text_ocr: "Image Text OCR", musicProvider: "Music Model", musicProviderTitle: "OneShowMusic generation service", musicProviderHint: "Manage the server-side music model used by AI Music Studio. Credentials are encrypted on the backend and never sent to browsers.",
    ...
    managed_runtime: "OneShowModel", market_intelligence: "Market Intelligence", oneshow_home_chat: "OneShow Home Chat", food_nutrition: "Food Nutrition Vision", modelName: "Configuration name", modelProtocol: "API protocol",
...
function PlatformModelsView({ data, locale, canManage, canManageStorage, onTest, onSave, onMusicTest, onMusicSave, onSingingTest, onSingingSave, onImageTest, onImageSave, onImageEditTest, onImageEditSave, onWorkspaceTest, onWorkspaceSave, onStorageTest, onStorageSave, onStockMarketTest, onStockMarketSave }) {
  ...
  const purposes = ["managed_runtime", "market_intelligence", "oneshow_home_chat", "food_nutrition", "model_studio_workspace", "stock_market_data", "music_generation", "singing_cover", "image_generation", "image_editing", "image_text_ocr", "image_upscaling", "storage_management"];
  return <div className="admin-page-stack platform-model-page">
    ...
    {purpose === "stock_market_data"
      ...
      : ["image_editing", "image_upscaling", "image_text_ocr"].includes(purpose)
      ? <section className="platform-model-layout"><ImageEditProviderView purpose={purpose} configuration={purpose === "image_editing" ? data?.imageEditing : purpose === "image_text_ocr" ? data?.imageTextOcr : data?.imageUpscaling} workspace={data?.modelStudioWorkspace} locale={locale} canManage={canManage} onTest={onImageEditTest} onSave={onImageEditSave} /></section>
...
function ImageEditProviderView({ purpose, configuration, workspace, locale, canManage, onTest, onSave }) {
  ...
  useEffect(() => {
    ...
    const defaultModel = purpose === "image_text_ocr" ? "qwen-vl-ocr-latest" : "qwen-image-2.0";
    setDraft({ adapter, credentialSource: configuration?.credentialSource || (workspace?.configured ? "workspace" : "direct"), baseUrl: configuration?.baseUrl || "https://dashscope.aliyuncs.com/api/v1", modelId: configuration?.modelId || defaultModel, apiKey: "", creditCost: configuration?.creditCost || (purpose === "image_text_ocr" ? 1 : purpose === "image_upscaling" ? 20 : 30), status: configuration?.enabled === false && configuration?.configured ? "disabled" : "active", reason: "" });
    setTestResult(null);
  }, [configuration?.updatedAt, configuration?.configured, purpose, workspace?.updatedAt, workspace?.configured]);
  const change = (key) => (event) => setDraft((current) => ({ ...current, [key]: event.target.value }));
  const defaultModel = purpose === "image_text_ocr" ? "qwen-vl-ocr-latest" : "qwen-image-2.0";
  const changeCredentialSource = (event) => { const credentialSource = event.target.value; setDraft((current) => credentialSource === "workspace" ? { ...current, credentialSource, adapter: "dashscope", baseUrl: workspace?.baseUrl || "https://dashscope.aliyuncs.com/api/v1", modelId: defaultModel, apiKey: "" } : { ...current, credentialSource: "direct", adapter: "dashscope", baseUrl: "https://dashscope.aliyuncs.com/api/v1", modelId: defaultModel }); };
  ...
  const title = purpose === "image_text_ocr" ? (locale === "en" ? "Image text recognition" : "图片文字识别") : purpose === "image_upscaling" ? (locale === "en" ? "Image restoration and upscale" : "图片高清修复") : (locale === "en" ? "AI image editing" : "AI 图片编辑");
  return <><article className="admin-v2-panel platform-model-editor"><div className="platform-model-current"><span className={`admin-metric-state ${configuration?.configured && configuration?.enabled ? "healthy" : "warning"}`} /><div><strong>{configuration?.configured ? configuration.modelId : t.notReporting}</strong><small>{configuration?.configured ? `${configuration.keyHint || "••••"} · ${configuration.lastTestStatus || t.pending}` : (locale === "en" ? "No provider configured" : "尚未配置模型")}</small></div></div><form className="platform-model-form" onSubmit={save}>
    <label>{locale === "en" ? "Credential source" : "连接来源"}<select value={draft.credentialSource || "direct"} onChange={changeCredentialSource} disabled={!canManage}><option value="workspace" disabled={!workspace?.configured}>{locale === "en" ? "Inherit default Model Studio workspace" : "继承默认百炼工作空间"}</option><option value="direct">{locale === "en" ? "Independent API key" : "使用独立 API Key"}</option></select></label>
    <label>{locale === "en" ? "API format" : "接口格式"}<select value={draft.adapter || "dashscope"} onChange={changeAdapter} disabled={!canManage || purpose === "image_text_ocr"}><option value="dashscope">Alibaba Cloud Model Studio</option>{purpose !== "image_text_ocr" && <option value="openai">OpenAI Images</option>}</select></label>
    <label>{t.modelId}<input list={`image-model-options-${purpose}`} value={draft.modelId || ""} onChange={change("modelId")} disabled={!canManage} required /><datalist id={`image-model-options-${purpose}`}>{purpose === "image_text_ocr" ? <><option value="qwen-vl-ocr-latest" /><option value="qwen-vl-ocr-2025-11-20" /></> : <><option value="qwen-image-2.0-pro-2026-06-22" /><option value="qwen-image-2.0-pro" /><option value="qwen-image-2.0" /><option value="qwen-image-edit-max" /><option value="qwen-image-edit-plus" /></>}</datalist></label>
    <label className="wide">{t.modelBaseUrl}<input type="url" value={draft.credentialSource === "workspace" ? (workspace?.baseUrl || draft.baseUrl || "") : (draft.baseUrl || "")} onChange={change("baseUrl")} disabled={!canManage || draft.credentialSource === "workspace"} required /></label>
    {purpose !== "image_text_ocr" && <label>{locale === "en" ? "Credits per result" : "每次处理积分"}<input type="number" min="1" max="10000" value={draft.creditCost || 20} onChange={change("creditCost")} disabled={!canManage} required /></label>}
    <label>{t.musicStatus}<select value={draft.status || "active"} onChange={change("status")} disabled={!canManage}><option value="active">{t.musicActive}</option><option value="disabled">{t.musicDisabled}</option></select></label>
    {draft.credentialSource === "direct" && <label className="wide">{t.replaceApiKey}<input type="password" autoComplete="new-password" value={draft.apiKey || ""} onChange={change("apiKey")} disabled={!canManage} required={!configuration?.configured || configuration?.credentialSource === "workspace"} /></label>}
    <label className="wide">{t.changeReason}<input value={draft.reason || ""} onChange={change("reason")} disabled={!canManage} required /></label>
    {testResult && <div className="platform-model-test healthy"><CheckCircle size={17} /><span>{t.modelTestHealthy}</span><em>{testResult.latencyMs} ms</em></div>}
    {canManage && <div className="platform-model-actions"><button type="button" onClick={test} disabled={testing}>{testing ? <SpinnerGap className="spin" size={16} /> : <Pulse size={16} />}{t.testModel}</button><button className="admin-primary" disabled={saving}>{saving ? <SpinnerGap className="spin" size={16} /> : <LockKey size={16} />}{t.saveModel}</button></div>}
  </form></article><aside className="admin-v2-panel platform-model-guidance"><ImageSquare size={25} /><h3>{title}</h3>{purpose === "image_text_ocr" ? <><p>{locale === "en" ? "Uses the encrypted Model Studio workspace key to detect text positions and approximate visual styles before editing." : "复用百炼工作空间的加密密钥，为图片改字识别文字位置并分析基础样式。"}</p><ul><li>{locale === "en" ? "Recommended: qwen-vl-ocr-latest" : "推荐模型：qwen-vl-ocr-latest"}</li><li>{locale === "en" ? "Falls back to local OCR if the model is unavailable" : "模型不可用时自动回退本地 OCR，不影响上传"}</li><li>{locale === "en" ? "Credentials remain server-side" : "密钥始终只保存在服务端"}</li></ul></> : <><p>{locale === "en" ? "One encrypted server-side connection powers the commercial image tools. Generated files are copied immediately into private user storage." : "一个后端加密连接统一服务商业图片工具；模型结果会立即转存到用户私有文件中心与 OSS。"}</p><ul><li>{locale === "en" ? "Stable default: qwen-image-2.0; quality: qwen-image-2.0-pro" : "稳定默认：qwen-image-2.0；画质优先：qwen-image-2.0-pro"}</li><li>{locale === "en" ? "A successful test can be saved for 10 minutes without another model call" : "测试成功后 10 分钟内保存不会重复调用模型"}</li><li>{locale === "en" ? "The upscale tools fall back to the editing model when needed" : "未单独配置高清修复时，会安全回退到图片编辑模型"}</li></ul></>}</aside></>;
}
...
function PaymentProviderEditor({ provider, locale, canManage, onTest, onSave, onToggle }) {
  ...
  useEffect(() => {
    setDraft({
      ...
      apiV3Key: "", wechatpayPublicKey: "", wechatpayPublicKeyId: "", reason: "",
    });
    setTested(false);
  ...
  const toggle = async (event) => {
    ...
    setToggling(true);
    try { await onToggle(provider.id, enabled); } finally { setToggling(false); }
  };
  return <article className="admin-v2-panel payment-provider-editor">
    <header><div><small>{isAlipay ? "ALIPAY RSA2" : "WECHAT PAY API V3"}</small><h2>{isAlipay ? "支付宝" : "微信支付"}</h2><p>{locale === "en" ? "Credentials are encrypted and never returned to the browser." : "密钥加密保存，保存后不会再次返回浏览器。"}</p></div><div className="payment-channel-control"><span>{provider.enabled ? (locale === "en" ? "Accepting payments" : "允许收款") : provider.configured ? (locale === "en" ? "New orders disabled" : "已停止新订单") : (locale === "en" ? "Configure first" : "请先完成配置")}</span><label className="payment-channel-switch" title={!provider.configured ? (locale === "en" ? "Save credentials before enabling" : "保存支付配置后才能启用") : ""}><input type="checkbox" checked={Boolean(provider.enabled)} onChange={toggle} disabled={!canManage || !provider.configured || toggling} /><i /></label></div></header>
...
function CommerceView({ data, locale, onApprove, canManage, onProviderTest, onProviderSave, onProviderToggle }) {
  const t = copy[locale];
  ...
  return <div className="admin-page-stack"><div className="admin-provider-strip">{data?.providers?.map((provider) => <div key={provider.id}><span className={provider.enabled ? "ok" : provider.configured ? "warn" : "off"} /><strong>{provider.id}</strong><small>{provider.enabled ? "enabled" : provider.configured ? "configured" : "not configured"}</small></div>)}</div>
    <section className="admin-v2-panel admin-table-panel"><nav className="admin-section-tabs"><button className={tab === "channels" ? "active" : ""} onClick={() => setTab("channels")}>{locale === "en" ? "Payment channels" : "支付通道"} <span>{data?.providers?.filter((item) => item.enabled).length || 0}</span></button>{Object.keys(sections).map((key) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{t[key]} <span>{sections[key]?.length || 0}</span></button>)}</nav>
      {tab === "channels" ? <div className="payment-provider-grid">{(data?.providers || []).filter((item) => item.id !== "stripe").map((provider) => <PaymentProviderEditor key={provider.id} provider={provider} locale={locale} canManage={canManage} onTest={onProviderTest} onSave={onProviderSave} onToggle={onProviderToggle} />)}<article className="admin-v2-panel payment-provider-note"><ShieldCheck size={25} /><h3>{locale === "en" ? "Commercial safeguards" : "商业支付安全规则"}</h3><ul><li>{locale === "en" ? "Channels are disabled by default" : "支付通道默认关闭，配置完成后再单独开启"}</li><li>{locale === "en" ? "Disabling stops new orders without losing valid pending callbacks" : "关闭开关只停止新订单，已有订单仍可安全到账"}</li><li>{locale === "en" ? "Credits are granted only after verified server notifications" : "只有服务端验签成功后才发放积分"}</li><li>{locale === "en" ? "Duplicate callbacks never grant twice" : "重复回调不会重复到账"}</li></ul></article></div> : <div className="admin-v2-table-wrap"><table><thead><tr><th>ID / {t.account}</th><th>{t.status}</th><th>{t.provider}</th><th>{t.time}</th><th /></tr></thead><tbody>
...
function PromotionCenterView({ data, locale, canManage, onCreate, onStatus }) {
  ...
  const openChannelSetup = () => { setTab("channels"); setForm({ kind: "channel" }); };
  const openCampaignSetup = () => { setTab("campaigns"); setForm({ kind: "campaign" }); };
  const openLinkSetup = () => {
    if (!hasChannel) return openChannelSetup();
    setTab("links");
    setForm({ kind: "link", destinationUrl: "https://oneshowtools.com/#tools", channelId: channels[0]?.id || "" });
  };
  ...
  const journey = [
    { key: "channel", title: zh ? "创建推广渠道" : "Create a channel", body: zh ? "区分牛客、小红书、抖音等流量来源" : "Identify every traffic source", done: hasChannel, action: openChannelSetup, Icon: PaperPlaneTilt },
    { key: "campaign", title: zh ? "新建营销活动（可选）" : "Create a campaign (optional)", body: zh ? "将同一主题的推广归类，方便整体复盘" : "Group related promotions for analysis", done: hasCampaign, action: openCampaignSetup, Icon: Gift },
    { key: "link", title: zh ? "生成推广链接" : "Generate a tracking link", body: zh ? "复制链接开始投放并追踪真实转化" : "Publish a link and track conversion", done: hasLink, action: openLinkSetup, Icon: LinkSimple },
  ];
  const activeJourney = hasLink ? 2 : hasChannel ? 2 : 0;
  return <div className="admin-page-stack promotion-center">
    <section className="promotion-hero admin-v2-panel"><div><small>GROWTH OPERATIONS</small><h2>{zh ? "推广中心" : "Promotion Center"}</h2><p>{zh ? "通过专属链接追踪用户全生命周期行为，驱动注册、下载、使用与支付转化。" : "Track the full customer journey through first-party promotion links."}</p></div><button type="button" onClick={() => setTab("funnel")}><BookOpenText size={17} />{zh ? "了解推广归因" : "Attribution guide"}<ArrowRight size={14} /></button></section>
    ...
    {showLaunchpad && <section className="promotion-journey" aria-label={zh ? "推广初始化步骤" : "Promotion setup steps"}>{journey.map(({ key, title, body, done, action, Icon }, index) => <button key={key} className={`${done ? "done" : ""} ${index === activeJourney ? "active" : ""}`} onClick={action}><span>{done ? <Check size={17} weight="bold" /> : index + 1}</span><Icon size={21} /><strong>{title}</strong><small>{body}</small><ArrowRight size={17} /></button>)}</section>}

    {tab === "overview" && showLaunchpad && <section className="promotion-launchpad">
      <article className="promotion-launch-main admin-v2-panel"><div className="promotion-launch-visual"><PaperPlaneTilt size={54} weight="duotone" /><LinkSimple size={24} /></div><small>{zh ? "从第一步开始" : "START WITH STEP ONE"}</small><h2>{hasChannel ? (zh ? "现在生成第一条推广链接" : "Create your first tracking link") : (zh ? "还没有推广渠道" : "No promotion channel yet")}</h2><p>{hasChannel ? (zh ? "渠道已经准备好。营销活动可以稍后补充，现在就可以生成专属链接开始追踪。" : "Your channel is ready. Campaigns are optional, so you can generate a link now.") : (zh ? "先创建一个推广渠道，定义流量来自哪里。之后即可生成专属链接并查看每个平台的真实效果。" : "Create a channel to identify where traffic comes from, then generate a tracking link.")}</p>{canManage ? <div><button className="admin-primary promotion-launch-primary" onClick={hasChannel ? openLinkSetup : openChannelSetup}><Plus size={18} />{hasChannel ? (zh ? "生成第一条链接" : "Create first link") : (zh ? "创建第一个渠道" : "Create first channel")}</button><button className="promotion-secondary" onClick={() => document.querySelector("#promotion-guide")?.scrollIntoView({ behavior: "smooth" })}><BookOpenText size={17} />{zh ? "查看使用指南" : "View guide"}</button></div> : <p className="promotion-readonly"><LockKey size={15} />{zh ? "当前账号仅可查看，请联系超级管理员完成初始化。" : "Your account is read-only. Ask a super administrator to finish setup."}</p>}<em>{zh ? "完成以上步骤后，推广数据会自动出现在总览中" : "Analytics will appear here automatically after setup"}</em></article>
      <aside className="promotion-guide-card admin-v2-panel" id="promotion-guide"><header><span><Eye size={20} /></span><div><h3>{zh ? "链接如何追踪效果？" : "How tracking works"}</h3><p>{zh ? "通过第一方短链接记录从访问到支付的完整路径" : "First-party links record the complete customer journey"}</p></div></header><div>{funnel.slice(0, 5).map(([label, , Icon], index) => <article key={label}><span><Icon size={18} /></span><div><strong>{label}</strong><small>{zh ? ["用户点击推广链接", "用户完成账号注册", "下载产品或客户端", "开始使用平台工具", "完成下单与支付"][index] : ["Clicks the promotion link", "Creates an account", "Downloads the product", "Uses a platform tool", "Completes a purchase"][index]}</small></div>{index < 4 && <i />}</article>)}</div><footer><ShieldCheck size={17} /><span>{zh ? "数据实时统计，可按渠道和活动复盘" : "Real-time analytics by channel and campaign"}</span></footer></aside>
    </section>}

    {tab === "overview" && !showLaunchpad && <>
    ...
    {tab === "links" && <section className="admin-v2-panel admin-table-panel"><header><div><small>TRACKABLE LINKS</small><h2>{zh ? "专属推广链接" : "Trackable promotion links"}</h2></div>{canManage && <button className="admin-primary" onClick={openLinkSetup}><Plus size={15} />{zh ? "新建链接" : "New link"}</button>}</header>
      {!hasChannel && <div className="promotion-prerequisite"><span><PaperPlaneTilt size={24} /></span><div><strong>{zh ? "需要先创建推广渠道" : "Create a channel first"}</strong><p>{zh ? "渠道用于标记流量来源。创建后即可在这里生成推广链接。" : "Channels identify traffic sources and are required for tracking links."}</p></div>{canManage && <button className="admin-primary" onClick={openChannelSetup}>{zh ? "去创建渠道" : "Create channel"}<ArrowRight size={15} /></button>}</div>}
      {form.kind === "link" && hasChannel && <div className="promotion-form admin-form-grid"><label>{zh ? "链接名称 *" : "Name *"}<input placeholder={zh ? "例如：小红书秋招笔记 01" : "e.g. Autumn hiring post 01"} value={form.name || ""} onChange={(e) => set("name", e.target.value)} /></label><label>{zh ? "落地页地址 *" : "Destination *"}<input placeholder="https://oneshowtools.com/#tools" value={form.destinationUrl || ""} onChange={(e) => set("destinationUrl", e.target.value)} /></label><label>{zh ? "渠道 *" : "Channel *"}<select value={form.channelId || ""} onChange={(e) => set("channelId", e.target.value)}><option value="">{zh ? "请选择渠道" : "Select channel"}</option>{channels.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>{zh ? "营销活动（可选）" : "Campaign (optional)"}<select value={form.campaignId || ""} onChange={(e) => set("campaignId", e.target.value)}><option value="">{zh ? "不归属活动" : "No campaign"}</option>{campaigns.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>{zh ? "内容标题（可选）" : "Content title (optional)"}<input placeholder={zh ? "用于区分不同文章或笔记" : "Identify a post or article"} value={form.contentTitle || ""} onChange={(e) => set("contentTitle", e.target.value)} /></label><label>{zh ? "自定义短码（可选）" : "Custom code (optional)"}<input placeholder={zh ? "留空则自动生成" : "Generated automatically if blank"} value={form.code || ""} onChange={(e) => set("code", e.target.value)} /></label><div className="promotion-form-actions"><button className="promotion-secondary" onClick={() => setForm({})}>{zh ? "取消" : "Cancel"}</button><button className="admin-primary" disabled={!form.name || !form.destinationUrl || !form.channelId} onClick={() => submit("links", form)}>{zh ? "生成推广链接" : "Create link"}<ArrowRight size={15} /></button></div></div>}
      <div className="admin-v2-table-wrap"><table><thead><tr><th>{zh ? "内容 / 链接" : "Content / Link"}</th><th>{zh ? "渠道" : "Channel"}</th><th>{zh ? "活动" : "Campaign"}</th><th>{zh ? "访客" : "Visitors"}</th><th>{zh ? "注册" : "Registrations"}</th><th>{zh ? "转化率" : "CVR"}</th><th /></tr></thead><tbody>{links.map((row) => <tr key={row.id}><td><strong>{row.name}</strong><small>{data.shortLinkBase}{row.code}</small></td><td>{row.channelName}</td><td>{row.campaignName || "—"}</td><td>{row.visitors}</td><td>{row.registrations}</td><td>{rate(row.registrations, row.visitors)}</td><td><button className="admin-link" onClick={() => copyLink(row.code)}><CopyIcon size={15} /></button><button className="admin-link" onClick={() => showQr(row)}><QrCode size={15} /></button>{canManage && <button className="admin-link" onClick={() => onStatus("links", row.id, row.status === "active" ? "inactive" : "active")}>{row.status === "active" ? (zh ? "停用" : "Disable") : (zh ? "启用" : "Enable")}</button>}</td></tr>)}{!links.length && <tr><td colSpan="7" className="admin-empty">{hasChannel ? (zh ? "还没有推广链接，点击右上角开始创建" : "No tracking links yet") : (zh ? "创建渠道后，推广链接会显示在这里" : "Links will appear after you create a channel")}</td></tr>}</tbody></table></div></section>}
...
export function AdminApp() {
  ...
  const togglePaymentProvider = async (provider, enabled) => {
    try {
      await api(`/api/admin/v1/payment-providers/${provider}/status`, json("PATCH", {
        enabled, reason: enabled ? "管理员启用支付通道" : "管理员关闭支付通道",
      }));
