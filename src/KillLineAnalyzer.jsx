...
const blankProduct = (index = 0) => ({ id: uid(), name: `产品 ${index + 1}`, logo: "", xValue: index + 1, yValue: 60, price: "", duration: "", description: "", note: "", highlight: false, visible: true, color: COLORS[index % COLORS.length], features: {} });
...
const newProject = (key = "blank") => {
  const source = templates[key] || templates.blank;
  return {
...
function safeProject(value) {
  try {
    ...
    const retainedProducts = value.products.filter((product) => !(product.description === "Demo 产品" && product.note === "示例数据，仅用于功能演示"));
    project.products = (retainedProducts.length ? retainedProducts : [blankProduct(0), blankProduct(1)]).slice(0, 50).map((product, index) => ({ ...blankProduct(index), ...product, id: String(product.id || uid()), xValue: Number(product.xValue) || 0, yValue: Number(product.yValue) || 0 }));
    if (value.template === "interview") project.template = "blank";
    if (value.title === "2026 AI 面试助手 价格 × 功能 斩杀线") project.title = templates.blank.title;
    if (value.subtitle === "示例数据，仅用于功能演示") project.subtitle = templates.blank.subtitle;
...
function spreadChartPoints(products, toX, toY) {
  ...
  return products.map((product) => {
    ...
    const labelSide = position.x > PLOT.left + (WIDTH - PLOT.left - PLOT.right) * .62 ? -1 : 1;
    return { ...product, anchorX, anchorY, plotX: position.x, plotY: position.y, labelSide, displaced: Math.hypot(position.x - anchorX, position.y - anchorY) > 3 };
  });
}
...
export function KillLineAnalyzer({ onBack }) {
  ...
  const chartProducts = useMemo(() => scoredProducts.filter((product) => product.visible !== false), [scoredProducts]);
  const logAllowed = chartProducts.every((product) => product.xValue > 0);
  const useLog = project.axes.xScale === "log" && logAllowed;
  const xDomain = zoomDomain(axisDomain(chartProducts.map((product) => product.xValue), useLog), zoom, useLog);
  ...
  const yDomain = zoomDomain(axisDomain([...chartProducts.map((product) => product.yValue), ...thresholdValues]), zoom);
  ...
  const statusProducts = chartProducts.map((product) => ({ ...product, pass: project.axes.yDirection === "higher" ? product.yValue >= lineValue(project, product.xValue) : product.yValue <= lineValue(project, product.xValue) }));
  ...
  useEffect(() => {
    ...
    const finalStage = 6 + chartProducts.length;
  ...
  }, [playing, stage, chartProducts.length, project.presentation]);
  ...
  return <main className={`kill-line-page ${presentation ? "is-presenting" : ""}`}>
    {!presentation && <><button className="kill-back" onClick={onBack}><ArrowLeft size={17} />返回工具市场</button><header className="kill-hero"><span className="kill-hero-icon"><img src="/tool-icons-v2/product-kill-line-analyzer-v1.png" alt="" /></span><div><small>PRODUCT KILL LINE · ONLINE</small><h1>斩杀线 · 二维产品竞争力分析器</h1><p>用两个关键维度看清产品位置，建立规则，画出真正有解释力的竞争力边界。</p></div><em>已上线</em></header></>}
    <section className="kill-workspace">
      {!presentation && <aside className="kill-config">
        ...
        {tab === "products" && <div className="kill-pane"><label>分析模板<select value={project.template} onChange={(event) => loadTemplate(event.target.value)}>{Object.entries(templates).map(([key, item]) => <option key={key} value={key}>{item.name}</option>)}</select></label><div className="kill-product-list">{project.products.map((product) => { const isVisible = product.visible !== false; return <article draggable className={`${selectedId === product.id ? "active" : ""} ${isVisible ? "" : "is-hidden"}`} key={product.id} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/product-id", product.id); }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => { event.preventDefault(); reorderProducts(event.dataTransfer.getData("text/product-id"), product.id); }} onClick={() => setSelectedId(product.id)}><i style={{ background: product.color }}>{product.logo ? <img src={product.logo} alt="" /> : initials(product.name)}</i><div className="kill-product-copy"><strong>{product.name}</strong><small>{project.axes.xName} {product.xValue} · {project.axes.yName} {scoreProduct(product, project).toFixed(1)}</small></div><div className="kill-product-actions"><button type="button" className={`kill-visibility-toggle ${isVisible ? "" : "off"}`} aria-label={`${isVisible ? "从图表隐藏" : "显示到图表"} ${product.name}`} aria-pressed={isVisible} title={isVisible ? "从图表隐藏" : "显示到图表"} onClick={(event) => { event.stopPropagation(); changeProduct(product.id, { visible: !isVisible }); if (isVisible && focusId === product.id) setFocusId(""); }}>{isVisible ? <Eye size={16} weight="fill" /> : <EyeSlash size={16} />}</button><button type="button" aria-label={`复制 ${product.name}`} title="复制" onClick={(event) => { event.stopPropagation(); duplicate(product); }}><Copy size={14} /></button><button type="button" aria-label={`删除 ${product.name}`} title="删除" onClick={(event) => { event.stopPropagation(); remove(product.id); }}><Trash size={14} /></button></div></article>; })}</div><p className="kill-help">点击眼睛控制产品是否进入曲线；隐藏后数据仍会保留。可拖动卡片调整演示顺序。</p><button className="kill-add" onClick={addProduct}><Plus size={15} />新增产品</button>{project.products.filter((item) => item.id === selectedId).map((product) => <div className="kill-product-form" key={product.id}><h3>编辑产品</h3><label>产品名称<input value={product.name} onChange={(event) => changeProduct(product.id, { name: event.target.value })} /></label><div className="kill-two"><label>X 值<input type="number" value={product.xValue} onChange={(event) => changeProduct(product.id, { xValue: Number(event.target.value) })} /></label><label>Y 值<input type="number" disabled={project.dimensions.length > 0} value={scoreProduct(product, project).toFixed(1)} onChange={(event) => changeProduct(product.id, { yValue: Number(event.target.value) })} /></label></div><div className="kill-two"><label>价格<input value={product.price} onChange={(event) => changeProduct(product.id, { price: event.target.value })} /></label><label>套餐<input value={product.duration} onChange={(event) => changeProduct(product.id, { duration: event.target.value })} /></label></div><label>备注<textarea value={product.note} onChange={(event) => changeProduct(product.id, { note: event.target.value })} /></label><label className="kill-logo"><UploadSimple size={16} />上传 Logo<input hidden type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(event) => uploadLogo(product, event.target.files?.[0])} /></label></div>)}</div>}
      ...
      <section className="kill-stage"><header className="kill-stage-tools"><div>{!presentation && <><button onClick={startPresentation}><PresentationChart size={16} />演示模式</button><select value={focusId} onChange={(event) => setFocusId(event.target.value)}><option value="">全部产品</option>{chartProducts.map((product) => <option key={product.id} value={product.id}>聚焦：{product.name}</option>)}</select><span className="kill-zoom"><button aria-label="缩小图表" onClick={() => setZoom((value) => Math.max(1, value - .25))}>−</button><b>{zoom.toFixed(2)}×</b><button aria-label="放大图表" onClick={() => setZoom((value) => Math.min(3, value + .25))}>＋</button></span></>}</div>{presentation && <div className="kill-presentation-tools"><button onClick={() => setPlaying((value) => !value)}>{playing ? <Pause /> : <Play />}{playing ? "暂停" : "继续"}</button><button onClick={() => { setStage(0); setPlaying(true); }}><ArrowsClockwise />重新播放</button><button onClick={() => { setPresentation(false); setStage(99); setPlaying(false); }}><X />退出演示</button></div>}</header>
        <div className="kill-chart-shell">
          <svg ref={svgRef} className="kill-chart" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={project.title} onClick={(event) => { if (project.line.mode === "manual" && !event.target.closest?.(".kill-point, .kill-control")) update(["line", "points"], [...project.line.points, fromPoint(event.clientX, event.clientY)]); }} onPointerMove={chartPointer} onPointerUp={() => setDragPoint(-1)} onPointerLeave={() => setDragPoint(-1)}>
            ...
            {layoutProducts.slice(0, stage >= 99 ? layoutProducts.length : visibleCount).map((product) => { const dimmed = focusId && focusId !== product.id; const killed = showRegions && !product.pass && project.presentation.killAnimation; const radius = focusId === product.id ? 28 : 23; const labelWidth = 106; const labelX = product.labelSide > 0 ? radius + 10 : -radius - 10 - labelWidth; const textX = product.labelSide > 0 ? labelX + 10 : labelX + labelWidth - 10; const textAnchor = product.labelSide > 0 ? "start" : "end"; return <g key={product.id}>{product.displaced && <><line x1={product.anchorX} y1={product.anchorY} x2={product.plotX} y2={product.plotY} className="kill-point-leader" /><circle cx={product.anchorX} cy={product.anchorY} r="3.5" className="kill-point-anchor" /></>}<g className={`kill-point ${selectedId === product.id ? "selected" : ""} ${dimmed ? "dimmed" : ""} ${killed ? "is-killed" : ""}`} transform={`translate(${product.plotX} ${product.plotY})`} onClick={(event) => { event.stopPropagation(); setSelectedId(product.id); setTab("products"); }}><title>{`${product.name}\n${project.axes.xName}: ${product.xValue} ${project.axes.xUnit}\n${project.axes.yName}: ${product.yValue.toFixed(1)} ${project.axes.yUnit}\n${product.price} ${product.duration}\n${product.note}`}</title><circle r={radius + 5} fill="#fff" stroke={product.highlight || product.id === topId ? "#10b981" : killed ? "#ef4444" : "#cbd5e1"} strokeWidth={product.highlight || product.id === topId ? 4 : 2} />{product.logo ? <image href={product.logo} x={-radius} y={-radius} width={radius * 2} height={radius * 2} preserveAspectRatio="xMidYMid meet" /> : <><circle r={radius} fill={product.color} /><text textAnchor="middle" y="5" className="kill-initials">{initials(product.name)}</text></>}<g className="kill-point-label"><rect x={labelX} y="-20" width={labelWidth} height="40" rx="9" /><text x={textX} y="-4" textAnchor={textAnchor} className="kill-product-name">{product.name}</text><text x={textX} y="12" textAnchor={textAnchor} className="kill-product-value">{product.xValue} · {product.yValue.toFixed(0)}{killed ? " · 被斩杀" : ""}</text></g>{product.id === topId && <g transform={`translate(${radius - 4} ${-radius - 4})`}><circle r="12" fill="#10b981" /><text textAnchor="middle" y="4" className="kill-top-check">★</text></g>}</g></g>; })}
