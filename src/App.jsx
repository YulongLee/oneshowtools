...
function Runtime({ data, dashboard, tasks = [], locale, onRefresh, onNotice, onNavigate, initialTab = "connections" }) {
  ...
  return <div className="runtime-dashboard-layout runtime-v3"><main className="runtime-page">
    ...
    {runtimeTab === "desktop" && <section className="runtime-section desktop-client-section">
      <header className="desktop-client-heading"><div><span>{isEn ? "DESKTOP EXPERIENCE" : "桌面端体验"}</span><h2>{isEn ? "Bring OneShowTools to your desktop" : "把 OneShowTools 装进电脑"}</h2><p>{isEn ? "A faster desktop workspace for AI tools, files and task notifications. Downloads will open after the clients are ready." : "更快捷地使用 AI 工具、拖入本地文件，并及时接收任务完成通知。客户端完成开发后将在这里开放下载。"}</p></div><span className="desktop-client-status"><span />{isEn ? "In development" : "开发中"}</span></header>
      <div className="desktop-client-cards">
        <article className="desktop-client-card macos"><div className="desktop-client-visual"><span className="desktop-platform-icon"><AppleLogo size={38} weight="fill" /></span><img src="/dashboard/dashboard-workbench-hero-v2.webp" alt="OneShowTools desktop workspace preview" /></div><div className="desktop-client-copy"><div><h3>OneShowTools for macOS</h3><span>{isEn ? "Planned" : "计划支持"}</span></div><p>{isEn ? "For Mac desktop computers, with Apple Silicon and Intel support planned." : "面向 Mac 桌面电脑，计划同时支持 Apple Silicon 与 Intel 芯片。"}</p><ul><li>macOS 13.0+</li><li>Apple Silicon / Intel</li></ul><button type="button" onClick={clientNotice}><Desktop size={17} />{isEn ? "Coming soon" : "即将开放"}<ArrowRight size={15} /></button></div></article>
        <article className="desktop-client-card windows"><div className="desktop-client-visual"><span className="desktop-platform-icon"><WindowsLogo size={38} weight="fill" /></span><img src="/runtime/oneshow-runtime-platform.webp" alt="OneShowTools Runtime desktop preview" /></div><div className="desktop-client-copy"><div><h3>OneShowTools for Windows</h3><span>{isEn ? "Planned" : "计划支持"}</span></div><p>{isEn ? "A native Windows workspace for AI tools, local files and background tasks." : "面向 Windows 的桌面工作台，集中使用 AI 工具、本地文件与后台任务。"}</p><ul><li>Windows 10 / 11</li><li>64-bit</li></ul><button type="button" onClick={clientNotice}><Desktop size={17} />{isEn ? "Coming soon" : "即将开放"}<ArrowRight size={15} /></button></div></article>
      </div>
      <div className="desktop-client-features">{[[Keyboard, isEn ? "Global shortcut" : "全局快捷键", isEn ? "Open tools from anywhere" : "随时唤起常用工具"],[Crop, isEn ? "Screenshot to AI" : "截图调用 AI", isEn ? "Capture and ask instantly" : "截图后直接提问处理"],[FolderOpen, isEn ? "Local files" : "本地文件", isEn ? "Drag, upload and process" : "拖入文件即可处理"],[Bell, isEn ? "Task notifications" : "任务通知", isEn ? "Know when work is done" : "任务完成及时提醒"],[ShieldCheck, isEn ? "Secure by design" : "数据安全", isEn ? "Protected local access" : "本地连接更安全"]].map(([Icon, title, body]) => <article key={title}><span><Icon size={21} weight="duotone" /></span><strong>{title}</strong><small>{body}</small></article>)}</div>
      <div className="desktop-mobile-roadmap"><span><DeviceMobile size={21} weight="duotone" /></span><div><strong>{isEn ? "iPhone and iPad are on the roadmap" : "iPhone 与 iPad 客户端已列入后续规划"}</strong><small>{isEn ? "iOS is a mobile platform and will be planned separately from the macOS desktop client." : "iOS 属于移动端，会与 macOS 桌面客户端分开规划和发布。"}</small></div><em>{isEn ? "Roadmap" : "后续开发"}</em></div>
      <footer className="desktop-client-help"><ShieldCheck size={16} weight="fill" /><span>{isEn ? "The page currently previews the product plan only. No installer is available yet." : "当前页面仅用于展示产品规划，暂未提供任何安装包。"}</span><a href="mailto:contact@oneshowailab.com">{isEn ? "Contact support" : "联系支持"}<ArrowRight size={13} /></a></footer>
    </section>}
...
export function App() {
  ...
  const navGroups = [
    ...
    { label: locale === "en" ? "ADVANCED" : "高级", items: [["agent", Robot, "Beta"], ["runtime", RocketLaunch], ["downloads", DownloadSimple, "NEW"]] },
