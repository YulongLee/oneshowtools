...
function Runtime({ data, dashboard, tasks = [], locale, onRefresh, onNotice, onNavigate, initialTab = "connections" }) {
  ...
  const switchRuntimeTab = (tab) => {
    if (tab === "desktop" && initialTab !== "desktop") return onNavigate?.("downloads");
    if (tab !== "desktop" && initialTab === "desktop") return onNavigate?.("runtime");
    setRuntimeTab(tab);
  };
  const clientNotice = () => onNotice(isEn ? "The desktop clients are in development. Downloads will open after the official release." : "桌面客户端正在开发中，正式发布后会在这里开放下载。");
  ...
  return <div className="runtime-dashboard-layout runtime-v3"><main className="runtime-page">
    ...
    {runtimeTab === "desktop" && <section className="runtime-section desktop-client-section">
      ...
      <section className="desktop-platform-panel"><header><div><span>{isEn ? "CHOOSE YOUR PLATFORM" : "选择你的平台"}</span><h3>{isEn ? "Desktop clients in active development" : "桌面客户端正在开发中"}</h3></div><span className="desktop-client-status"><i />{isEn ? "Release preview" : "发布预告"}</span></header><div className="desktop-platform-grid">
        <article className="macos"><span className="desktop-platform-icon"><AppleLogo size={28} weight="fill" /></span><div><small>{isEn ? "DESKTOP APP" : "桌面客户端"}</small><h4>OneShowTools for macOS</h4><p>{isEn ? "Native support is planned for both Apple Silicon and Intel Macs." : "面向 Mac 的桌面工作台，计划同时支持 Apple Silicon 与 Intel 芯片。"}</p><ul><li>macOS 13.0+</li><li>Apple Silicon</li><li>Intel</li></ul></div><button type="button" onClick={clientNotice}>{isEn ? "Coming soon" : "首发后开放下载"}<ArrowRight size={15} /></button></article>
        <article className="windows"><span className="desktop-platform-icon"><WindowsLogo size={28} weight="fill" /></span><div><small>{isEn ? "DESKTOP APP" : "桌面客户端"}</small><h4>OneShowTools for Windows</h4><p>{isEn ? "A focused Windows workspace for AI tools, local files and background tasks." : "面向 Windows 的原生体验，集中使用 AI 工具、本地文件与后台任务。"}</p><ul><li>Windows 10 / 11</li><li>64-bit</li></ul></div><button type="button" onClick={clientNotice}>{isEn ? "Coming soon" : "首发后开放下载"}<ArrowRight size={15} /></button></article>
      </div></section>
