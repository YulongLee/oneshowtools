...
const dictionary = {
  "zh-CN": {
    ...
    managedModel: "平台托管模型", personalModels: "我的模型连接", addModel: "添加模型连接", connectionName: "连接名称", providerTemplate: "模型服务商", baseUrl: "API 地址（Base URL）", baseUrlPlaceholder: "例如：https://api.example.com/v1", apiKey: "API Key", saveConnection: "安全保存",
  ...
  en: {
    ...
    managedModel: "Managed model", personalModels: "My model connections", addModel: "Add model connection", connectionName: "Connection name", providerTemplate: "Model provider", baseUrl: "API Base URL", baseUrlPlaceholder: "For example: https://api.example.com/v1", apiKey: "API Key", saveConnection: "Save securely",
...
function Runtime({ data, locale, onRefresh, onNotice }) {
  const t = dictionary[locale];
  const [form, setForm] = useState({ name: "", providerTemplate: "dashscope", baseUrl: "", modelId: "", apiKey: "" });
  const [busy, setBusy] = useState(false);
  ...
  if (!data) return <Loading locale={locale} />;
  const selectedTemplate = data.supportedTemplates.find((item) => item.id === form.providerTemplate);
  const requiresBaseUrl = Boolean(selectedTemplate?.requiresBaseUrl);
  ...
  const submit = async (event) => {
    ...
    if (saved) {
      setForm({ name: "", providerTemplate: "dashscope", baseUrl: "", modelId: "", apiKey: "" });
      setTestResult(null);
      setShowForm(false);
    }
  };
  ...
  const updateProvider = (providerTemplate) => {
    setForm((current) => ({ ...current, providerTemplate, baseUrl: "" }));
    setTestResult(null);
  };
