...
function SeoWorkbenchPage({ tool, catalog, locale, authenticated, runtime, onBack, onAuth, onCompleted, onModelChange }) {
  ...
  const labels = zh ? {
    ...
    run: "开始真实分析", running: "正在抓取与分析", waiting: "网站诊断可能需要 30–90 秒，请保持页面开启", empty: "选择任务并填写参数，报告与证据会显示在这里。", copy: "复制 Markdown", download: "下载 .md", downloadHtml: "下载 HTML", copied: "已复制", required: "请填写所有必填项", login: "登录后即可分析并保存任务记录", credits: "积分 / 次", locked: "需要数据源", lockedBody: "该能力依赖真实关键词、SERP 或外链供应商，配置后才会开放。", score: "规则评分", source: "数据来源", quality: "数据质量", error: "分析失败，请检查网址、数据源或模型配置。",
  } : {
  ...
  const setFieldValue = (fieldId, value) => setDraftsByTemplate((previous) => ({ ...previous, [templateId]: { ...(previous[templateId] || {}), [fieldId]: value } }));
  ...
  return <div className="writer-page seo-page">
    ...
    <div className="writer-shell seo-shell">
      ...
      <main className="writer-canvas">
        ...
        <section className="writer-editor"><header><span className={`writer-template-mark ${activeModule.accent}`}><MagnifyingGlass size={22} weight="duotone" /></span><div><small>{activeModule.label[zh ? "zh" : "en"]}</small><h2>{activeTemplate.label[zh ? "zh" : "en"]}</h2></div>{!activeTemplate.available && <span className="seo-source-required"><LockKey size={14} />{labels.locked}</span>}</header><div className="writer-fields">{activeTemplate.fields.map((field) => <label key={field.id} className={field.type === "textarea" ? "wide" : ""}><span>{field.label[zh ? "zh" : "en"]}{field.required && <em>*</em>}</span>{field.type === "textarea" ? <textarea rows={field.id === "content" ? 10 : 5} value={values[field.id] || ""} onChange={(event) => setFieldValue(field.id, event.target.value)} placeholder={field.placeholder?.[zh ? "zh" : "en"] || ""} /> : <input type={field.type === "url" ? "url" : "text"} value={values[field.id] || ""} onChange={(event) => setFieldValue(field.id, event.target.value)} placeholder={field.placeholder?.[zh ? "zh" : "en"] || ""} />}</label>)}</div></section>
        <section className="writer-result seo-result"><header><div><span className="writer-result-icon"><ChartLineUp size={19} weight="fill" /></span><div><small>MARKDOWN + EVIDENCE</small><h2>{labels.output}</h2></div></div>{result && <div className="writer-result-actions"><button onClick={copy}><Copy size={16} />{copied ? labels.copied : labels.copy}</button><button onClick={download}><DownloadSimple size={16} />{labels.download}</button>{result.html && <button onClick={downloadHtml}><DownloadSimple size={16} />{labels.downloadHtml}</button>}</div>}</header>{!result ? <div className="writer-result-empty"><ChartLineUp size={35} weight="duotone" /><strong>{labels.output}</strong><p>{labels.empty}</p></div> : <><div className="seo-result-metrics"><span><small>{labels.score}</small><strong>{result.score ?? "—"}{result.score != null ? "/100" : ""}</strong></span><span><small>{labels.source}</small><strong>{result.dataSource}</strong></span><span><small>{labels.quality}</small><strong>{result.dataQuality}</strong></span></div><pre>{result.markdown}</pre></>}</section>
      </main>
