...
function AgentHubPage({ tools, tasks, favorites, locale, onRun, onToggleFavorite, onNavigate }) {
  ...
  const agentTools = useMemo(() => {
    const dedicated = tools.filter((tool) => tool.category === "agent" || tool.slug?.includes("agent"));
    // When no dedicated Agent is published yet, every published tool remains a
    // real executable template for a single-capability Agent. This keeps the
    // hub useful without exposing administrator-disabled products.
    return dedicated.length ? dedicated : tools;
  }, [tools]);
  ...
  const copy = isEn ? {
    title:"AI Agent",subtitle:"Let AI Agents execute multi-step tasks and turn repeatable work into reliable workflows.",heroTitle:"Create your dedicated AI Agent",heroBody:"Start from a published template, set the goal and let the Agent run every step with a traceable task record.",create:"Create Agent",featured:"Featured Agents",featuredSub:"Published Agent capabilities ready to run",mine:"My Agents",mineSub:"Agents you have run recently",all:"View all",guide:"How AI Agents work",overview:"Agent overview",runs:"Runs",success:"Success rate",quick:"Quick actions",browse:"Browse templates",projects:"Open projects",templates:"Popular templates",empty:"No Agent has been run yet.",choose:"Choose an Agent template",chooseBody:"Every option below is a published tool and opens its real workflow.",close:"Close",use:"Use template",running:"runs",categories:"Agent categories",
  } : {
    title:"AI Agent",subtitle:"让 AI Agent 自动执行多步骤任务，把重复工作变成稳定、可追踪的工作流。",heroTitle:"创建你的专属 AI Agent",heroBody:"从真实模板开始，设置目标后由 Agent 完成每个步骤，并在任务中心保留完整执行记录。",create:"创建新 Agent",featured:"精选 Agent",featuredSub:"已经发布、可以真实运行的 Agent 能力",mine:"我的 Agent",mineSub:"你最近实际运行过的 Agent",all:"查看全部",guide:"如何使用 AI Agent",overview:"Agent 运行概览",runs:"运行次数",success:"成功率",quick:"快捷操作",browse:"浏览 Agent 模板",projects:"打开项目中心",templates:"热门模板",empty:"你还没有运行过 Agent。",choose:"选择 Agent 模板",chooseBody:"下面均为已经发布的真实工具，选择后会直接进入对应工作流。",close:"关闭",use:"使用模板",running:"次运行",categories:"Agent 分类",
  };
...
function FavoritesPage({ tools, tasks, files, favorites, collections, counts, locale, onRun, onOpenTask, onNavigate, onAdd, onRemove, onMove, onCreateCollection }) {
  ...
  const visible = favoriteItems.filter((favorite) => favorite.itemType === tab && searchable(favorite).toLowerCase().includes(query.trim().toLowerCase()))
  ...
  const itemIcon = (favorite, size=25) => favorite.itemType === "tool" ? <ProductToolIcon tool={favorite.item} size={size}/>
    : favorite.itemType === "prompt" ? <span className="favorite-kind-icon prompt"><NotePencil size={size} weight="duotone"/></span>
      : favorite.item.mimeType?.startsWith("image/") ? <span className="favorite-kind-icon image"><img src={`/api/files/${favorite.item.id}/download`} alt="" loading="lazy"/></span>
  ...
  return <div className="favorites-page page-stack">
    ...
    <nav className="favorites-tabs">{tabs.map(([id,label,Icon])=><button className={tab===id?"active":""} key={id} onClick={()=>setTab(id)}><Icon size={16}/>{label}<small>{counts?.[id]||0}</small></button>)}</nav>
    <section className="surface favorites-library"><main><header><nav><button className="active">{copy.all}<small>{visible.length}</small></button><button onClick={()=>setShowFolders(true)}>{copy.folders}<small>{collections.length}</small></button></nav><label><MagnifyingGlass size={15}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder={copy.search}/></label></header>
      {visible.length ? <div className={`favorite-card-grid ${layout}`}>{visible.map((favorite)=><article key={favorite.id} className="favorite-card">{itemIcon(favorite,28)}<div className="favorite-card-copy"><strong title={labelFor(favorite)}>{labelFor(favorite)}</strong><small>{favorite.itemType === "tool" ? (isEn ? favorite.item.descriptionEn : favorite.item.descriptionZh) : favorite.itemType === "prompt" ? (isEn ? favorite.item.toolNameEn : favorite.item.toolNameZh) : `${formatBytes(favorite.item.sizeBytes)} · ${favorite.item.sourceNameZh || copy.file}`}</small>{favorite.itemType === "tool" && <span>{favorite.item.tags?.slice(0,2).map((tag)=><em key={tag}>{tag}</em>)}</span>}</div><footer><time>{formatDate(favorite.createdAt,locale)}</time><label title={copy.move}><select value={favorite.collectionId||""} onChange={(event)=>onMove(favorite.id,event.target.value||null)}><option value="">{copy.root}</option>{collections.map((folder)=><option value={folder.id} key={folder.id}>{folder.name}</option>)}</select></label><button onClick={()=>openFavorite(favorite)}><ArrowRight size={15}/></button><button className="favorite-remove" title={copy.remove} onClick={()=>onRemove(favorite.id)}><Star size={16} weight="fill"/></button></footer></article>)}</div> : <EmptyState
        icon={Star}
        title={copy.empty}
        action={<button className="primary-button" onClick={()=>onNavigate("marketplace")}>{copy.browse}</button>}
      />}
      {collections.length>0&&<section className="favorite-folders"><h2>{copy.folders}</h2><div>{collections.map((folder)=><article key={folder.id}><FolderOpen size={23} weight="duotone"/><span><strong>{folder.name}</strong><small>{folder.itemCount} {isEn?"items":"项收藏"}</small></span></article>)}</div></section>}
...
function ProjectCenter({ projects, locale, onNavigate, onRun, tools }) {
  ...
  const openProject = () => seoAgent ? onRun(seoAgent) : onNavigate("agent");
  const projectState = (project) => {
    if (project.status === "archived") return "archived";
    if (["queued", "running"].includes(project.latestScan?.status)) return "running";
    if (project.latestScan?.status === "completed") return "completed";
    return "active";
  };
  ...
  const scheduled = projects.filter((project) => project.nextScanAt && new Date(project.nextScanAt).getTime() > Date.now()).length;
  const copy = isEn ? {
    title: "Project Center", subtitle: "Manage and track all of your AI projects in one place.", all: "All projects", created: "Created by me", involved: "Active", inspected: "Inspected", archived: "Archived", newProject: "New project", search: "Search project name or website", allStatus: "All status", newest: "Recently updated", oldest: "Oldest updated", score: "Health score", total: "Projects", active: "Active", completed: "Inspected", scheduled: "Scheduled", recent: "Recent activity", open: "Open project", templates: "Quick starts", seoTemplate: "Website SEO project", agentTemplate: "AI Agent workspace", taskTemplate: "Task Center", lastUpdate: "Updated", noProjects: "No matching projects", noProjectsBody: "Create your first project to organize scans, reports, tasks and files.", automation: "Automation", manual: "Manual", health: "Health", status: { active: "Active", running: "Scanning", completed: "Inspected", archived: "Archived" },
  } : {
    title: "项目中心", subtitle: "集中管理和跟踪你的所有 AI 项目，提升协作效率。", all: "全部项目", created: "我创建的", involved: "进行中", inspected: "已巡检", archived: "已归档", newProject: "新建项目", search: "搜索项目名称或网站", allStatus: "全部状态", newest: "最近更新", oldest: "最早更新", score: "健康评分", total: "全部项目", active: "进行中", completed: "已巡检", scheduled: "待巡检", recent: "最近动态", open: "打开项目", templates: "常用入口", seoTemplate: "网站 SEO 项目", agentTemplate: "AI Agent 工作区", taskTemplate: "任务中心", lastUpdate: "最近更新", noProjects: "没有符合条件的项目", noProjectsBody: "创建第一个项目，把巡检、报告、任务和文件集中管理。", automation: "自动巡检", manual: "手动运行", health: "健康度", status: { active: "进行中", running: "巡检中", completed: "已巡检", archived: "已归档" },
  };
  const tabs = [["all", copy.all], ["created", copy.created], ["active", copy.involved], ["completed", copy.inspected], ["archived", copy.archived]];
  const visible = projects.filter((project) => {
    const matchesQuery = `${project.name || ""} ${project.siteOrigin || ""} ${project.siteUrl || ""}`.toLowerCase().includes(query.trim().toLowerCase());
    ...
    if (sort === "score") return Number(b.latestScan?.healthScore || 0) - Number(a.latestScan?.healthScore || 0);
  ...
  return <div className="projects-dashboard-layout">
    <main className="projects-page">
      <header className="projects-heading"><div><h1>{copy.title}</h1><p>{copy.subtitle}</p></div><button className="primary-button" onClick={openProject}><Plus size={16}/>{copy.newProject}<CaretDown size={13}/></button></header>
      ...
      {visible.length ? <section className={`project-card-grid-v2 ${layout}`}>{visible.map((project) => {
        const state = projectState(project);
        const health = Number(project.latestScan?.healthScore || 0);
        return <article className="surface project-card-v2" key={project.id}>
          <button className="project-card-main" onClick={openProject}>
            <div className="project-visual"><span><ChartLineUp size={34} weight="duotone"/></span><div><small>{project.siteOrigin || project.siteUrl || "OneShowSEO"}</small><strong>{health ? `${health}` : "SEO"}</strong><em>{health ? copy.health : "OneShowSEO"}</em></div></div>
            <div className="project-card-copy"><strong>{project.name}</strong><p>{project.siteOrigin || project.siteUrl}</p><div><span>SEO</span><span>{project.automationMode === "recommend" ? copy.automation : copy.manual}</span>{project.ownershipStatus && <span>{project.ownershipStatus}</span>}</div></div>
          </button>
          <footer><span className={`project-state ${state}`}><i/>{copy.status[state]}</span><time>{copy.lastUpdate} · {formatDate(project.updatedAt, locale)}</time><button onClick={openProject} aria-label={copy.open}><ArrowRight size={16}/></button></footer>
        </article>;
      })}</section> : <section className="surface projects-empty"><EmptyState icon={Database} title={copy.noProjects} body={copy.noProjectsBody} action={<button className="primary-button" onClick={openProject}><Plus size={15}/>{copy.newProject}</button>}/></section>}
    ...
    <aside className="projects-side">
      ...
      <section className="surface project-activity"><header><h2>{copy.recent}</h2></header>{recent.length ? recent.map((project) => <button key={project.id} onClick={openProject}><span><ChartLineUp size={17} weight="duotone"/></span><div><strong>{project.name}</strong><small>{copy.status[projectState(project)]}</small></div><time>{formatDate(project.updatedAt, locale)}</time></button>) : <p>{copy.noProjects}</p>}</section>
      <section className="surface project-quick"><header><h2>{copy.templates}</h2></header><button onClick={openProject}><span><ChartLineUp size={18} weight="duotone"/></span><div><strong>{copy.seoTemplate}</strong><small>OneShowSEO</small></div><ArrowRight size={15}/></button><button onClick={() => onNavigate("agent")}><span><Robot size={18} weight="duotone"/></span><div><strong>{copy.agentTemplate}</strong><small>AI Agent</small></div><ArrowRight size={15}/></button><button onClick={() => onNavigate("tasks")}><span><ListChecks size={18} weight="duotone"/></span><div><strong>{copy.taskTemplate}</strong><small>{isEn ? "Track execution" : "查看执行进度"}</small></div><ArrowRight size={15}/></button></section>
    </aside>
  </div>;
}
...
function Account({ user, health, credits, billing, locale, onLogout, onUserChange, onLocaleChange, onNotice, onNavigate }) {
  ...
  const savePreferences = (next = preferences) => {
    setPreferences(next);
    localStorage.setItem("oneshowtools-account-preferences", JSON.stringify(next));
    onNotice(copy.save);
  };
...
export function App() {
  ...
  const loadPrivate = useCallback(async () => {
    if (!session) return;
    const [dashboard, runtime, credits, billing, tasks, files, projectData, favoriteData] = await Promise.all([
      api("/api/dashboard"), api("/api/runtime/status"), api("/api/credits"), api("/api/billing/status"), api("/api/tasks"), api("/api/files"), api("/api/seo-agent").catch(() => ({ projects: [] })), api("/api/favorites").catch(() => ({ favorites: [], collections: [], counts: { tool: 0, file: 0, prompt: 0, material: 0 } })),
    ]);
    setPrivateData({ dashboard, runtime, credits, billing, tasks: tasks.tasks, files: files.files, projects: projectData.projects || [], favorites: favoriteData.favorites || [], favoriteCollections: favoriteData.collections || [], favoriteCounts: favoriteData.counts || { tool: 0, file: 0, prompt: 0, material: 0 }, fileQuota: files.quota || { used: files.files.length, limit: 100, remaining: Math.max(0, 100 - files.files.length) } });
  }, [session]);
