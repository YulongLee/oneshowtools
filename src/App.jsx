...
export function App() {
  ...
  const loadPrivate = useCallback(async () => {
    ...
    const [dashboard, runtime, credits, billing, tasks, files, projectData, favoriteData] = await Promise.all([
      api("/api/dashboard"), api("/api/runtime/status"), api("/api/credits"), api("/api/billing/status"), api("/api/tasks"), api("/api/files"), api("/api/projects").catch(() => ({ projects: [] })), api("/api/favorites").catch(() => ({ favorites: [], collections: [], counts: { tool: 0, file: 0, prompt: 0, material: 0 } })),
    ]);
