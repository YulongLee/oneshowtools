...
function Dashboard({ data, tools, runtime, locale, onNavigate, onSearch, onRun }) {
  const t = dictionary[locale];
  ...
  const readyTools = tools.filter((tool) => tool.runtimeStatus === "ready");
  ...
  const quickTools = recommended.slice(0, 4);
  const categoryItems = marketplaceCategories.filter((item) => item.id !== "all")
    .map((item) => ({ ...item, count: tools.filter((tool) => item.accepts.includes(tool.category)).length }))
    .filter((item) => item.count > 0).slice(0, 9);
  ...
  return <div className="page-stack dashboard-page">
    <section className="dashboard-hero">
      <div className="dashboard-hero-copy"><p className="eyebrow">ONESH​OWTOOLS WORKSPACE</p><h1>{greeting}，{userName}<Sparkle size={22} weight="fill" /></h1><p>{t.today}</p>
      ...
      <div className="dashboard-hero-art" aria-hidden="true"><img src="/dashboard/oneshowtools-ai-toolkit-900.png" alt="" /></div>
    </section>
    ...
    <section className="dashboard-content-grid">
      <article className="surface dashboard-quick"><header><div><span>QUICK ACCESS</span><h2>{isEn ? "Quick access" : "快捷入口"}</h2></div><button onClick={() => onNavigate("marketplace")}><Plus size={15} />{isEn ? "More" : "更多工具"}</button></header><div>{quickTools.map((tool) => { const Icon = iconMap[tool.icon] || Wrench; return <button key={tool.id} onClick={() => onRun(tool)}><span className={`tool-icon compact ${tool.category}`}><Icon size={19} /></span><div><strong>{isEn ? tool.nameEn : tool.nameZh}</strong><small>{tool.creditCost} {t.creditsUnit}</small></div><ArrowRight size={16} /></button>; })}</div></article>

      <article className="surface dashboard-recommended"><header><div><span>RECOMMENDED</span><h2>{isEn ? "Recommended for you" : "为你推荐"}</h2></div><button onClick={() => onNavigate("marketplace")}>{isEn ? "View all" : "查看全部"}<ArrowRight size={15} /></button></header><div>{recommended.map((tool) => { const Icon = iconMap[tool.icon] || Wrench; return <button key={tool.id} onClick={() => onRun(tool)}><span className={`tool-icon ${tool.category}`}><Icon size={21} /></span><h3>{isEn ? tool.nameEn : tool.nameZh}</h3><p>{isEn ? tool.descriptionEn : tool.descriptionZh}</p><footer><span><Coins size={13} />{tool.creditCost}</span><span><CheckCircle size={13} weight="fill" />{t.ready}</span></footer></button>; })}</div></article>

      <aside className="dashboard-side-stack">
        <article className="surface dashboard-runtime"><header><h2>{isEn ? "AI Runtime status" : "AI Runtime 状态"}</h2><span className={runtime?.managed?.status === "ready" ? "online" : "offline"}>{runtime?.managed?.status === "ready" ? t.online : t.notConfigured}</span></header><dl><div><dt><RocketLaunch size={18} />{isEn ? "Managed model" : "平台模型"}</dt><dd>{runtime?.managed?.status === "ready" ? t.ready : t.config}</dd></div><div><dt><PlugsConnected size={18} />{isEn ? "Personal models" : "个人模型"}</dt><dd>{activeConnections}</dd></div><div><dt><ListChecks size={18} />{isEn ? "Running tasks" : "执行中任务"}</dt><dd>{data.metrics.running}</dd></div></dl><button onClick={() => onNavigate("runtime")}>{isEn ? "Open runtime" : "进入运行中心"}<ArrowRight size={15} /></button></article>
        <article className="surface dashboard-recent"><header><h2>{t.recentTasks}</h2><button onClick={() => onNavigate("tasks")}>{isEn ? "All" : "全部"}<ArrowRight size={14} /></button></header>{data.recentTasks.length ? <div>{data.recentTasks.slice(0, 4).map((task) => <TaskRow task={task} locale={locale} key={task.id} />)}</div> : <p>{t.noTasksHint}</p>}</article>
      </aside>
    </section>

    <section className="surface dashboard-explore"><header><span>EXPLORE</span><h2>{isEn ? "Explore more possibilities" : "探索更多可能"}</h2><p>{isEn ? "Discover tools and agents by capability" : "按能力发现更多 AI 工具和智能体"}</p></header><div>{categoryItems.map((item) => { const CategoryIcon = item.icon; return <button key={item.id} onClick={() => onNavigate("marketplace")}><span><CategoryIcon size={20} /></span><strong>{t[item.id]}</strong><small>{item.count}</small></button>; })}</div></section>
  </div>;
}

function Marketplace({ tools, locale, query, onQuery, onRun, data, runtime, tasks = [], onNavigate }) {
  const t = dictionary[locale];
  ...
  return <div className="marketplace-page">
    <div className="marketplace-primary">
      ...
      <section className="marketplace-featured surface">
        ...
        <div>{featured.map((tool, index) => { const Icon = iconMap[tool.icon] || Wrench; return <button className={`featured-tool tone-${index + 1}`} key={tool.id} onClick={() => onRun(tool)}><span className="featured-copy"><small>{index === 0 ? "NEW" : index === 1 ? "AGENT" : t.ready}</small><strong>{locale === "en" ? tool.nameEn : tool.nameZh}</strong><em>{locale === "en" ? tool.descriptionEn : tool.descriptionZh}</em><b>{isEn ? "Try now" : "立即体验"}</b></span><span className="featured-icon"><Icon size={52} weight="duotone" /></span></button>; })}</div>
      </section>
      ...
      <section className="marketplace-catalog surface">
        ...
        <div className="marketplace-catalog-body">
          ...
          <div className="marketplace-card-area">
            ...
            {visible.length ? <div className="marketplace-tool-grid">{visible.map((tool) => { const Icon = iconMap[tool.icon] || Wrench; const ready = tool.runtimeStatus === "ready"; return <article className="marketplace-tool-card" key={tool.id}><div className="tool-card-head"><span className={`tool-icon ${tool.category}`}><Icon size={22} weight="duotone" /></span>{ready ? <span className="tool-ready"><CheckCircle size={13} weight="fill" />{t.ready}</span> : <StatusPill status={tool.runtimeStatus} locale={locale} />}</div><h3>{locale === "en" ? tool.nameEn : tool.nameZh}</h3><p>{locale === "en" ? tool.descriptionEn : tool.descriptionZh}</p><div className="tool-card-tags"><span>{isEn ? "Tool" : "工具"}</span><span>{t[tool.category] || tool.category}</span></div><footer><strong><Coins size={14} />{tool.creditCost} {t.creditsUnit}</strong><button onClick={() => onRun(tool)}>{isEn ? "Use" : "使用"}</button></footer></article>; })}</div> : <EmptyState icon={selectedCategory.icon} title={query ? t.noResults : t.comingSoon} body={query ? undefined : t.comingSoonHint} action={!query && <button className="secondary-button" onClick={() => setCategory("all")}>{t.all}</button>} />}
...
function Runtime({ data, dashboard, tasks = [], locale, onRefresh, onNotice, onNavigate }) {
  ...
  const configurableTools = orderedTools.filter((tool) => tool.modelConfigurable);
  const recentTasks = tasks.slice(0, 4);
  const runtimeMetrics = [
    [isEn ? "Supported tools" : "支持工具", data.tools.length, Wrench],
    [isEn ? "Personal models" : "个人模型", activeConnections.length, PlugsConnected],
    [isEn ? "Running tasks" : "运行任务", dashboard?.metrics?.running || 0, ListChecks],
    [isEn ? "Completed" : "已完成", dashboard?.metrics?.completed || 0, CheckCircle],
  ];
  const tabs = [
  ...
  const connectionRows = <div className="runtime-connection-list">
    <article className="runtime-managed-row"><span className="runtime-model-logo"><img src="/brand/oneshowtools-mark.png" alt="" /></span><div><h3>OneShowModel <small>{isEn ? "Official" : "官方"}</small></h3><p>{t.managedDescription}</p><span className={data.managed.configured ? "runtime-connected" : "runtime-disconnected"}><CheckCircle size={13} weight="fill" />{data.managed.configured ? t.runtimeReady : t.notConfigured}</span></div><strong>{isEn ? "Default" : "默认"}</strong></article>
    {data.connections.map((connection) => <article className="runtime-connection-row" key={connection.id}><span className="connection-icon"><PlugsConnected size={20} /></span><div className="connection-copy"><h3>{connection.name}</h3><p>{connection.modelId} · {connection.keyHint}</p><small className="connection-endpoint">{connection.baseUrl}</small></div><span className={connection.lastTestStatus === "healthy" ? "runtime-connected" : "runtime-disconnected"}><span className={`connection-state ${connection.lastTestStatus === "healthy" ? "active" : connection.status}`} />{modelTestLabel(connection.lastTestStatus, locale)}{connection.lastTestLatencyMs ? ` · ${connection.lastTestLatencyMs}ms` : ""}</span><div className="connection-actions"><button disabled={busy} onClick={() => testSavedConnection(connection)}>{t.testConnection}</button><button disabled={busy} onClick={() => { const apiKey = window.prompt(t.apiKey); if (apiKey) mutate(`/api/model-connections/${connection.id}/rotate`, jsonOptions("POST", { apiKey }), t.configured); }}>{t.rotateKey}</button><button disabled={busy} onClick={() => mutate(`/api/model-connections/${connection.id}`, jsonOptions("PATCH", { status: connection.status === "active" ? "disabled" : "active" }), t.configured)}>{connection.status === "active" ? t.disable : t.enable}</button><button className="danger-link" disabled={busy} onClick={() => mutate(`/api/model-connections/${connection.id}`, { method: "DELETE" }, t.deleteConnection)}>{t.deleteConnection}</button></div></article>)}
    {!data.connections.length && <div className="runtime-empty-row"><PlugsConnected size={24} /><div><strong>{t.noConnections}</strong><p>{t.connectionsHint}</p></div></div>}
    {data.byokEnabled && <button className="runtime-add-row" onClick={() => setShowForm(true)}><Plus size={20} /><div><strong>{t.addModel}</strong><small>{isEn ? "OpenAI or Anthropic compatible endpoints" : "支持 OpenAI 或 Anthropic 兼容接口"}</small></div><ArrowRight size={18} /></button>}
  </div>;
  const routingRows = <div className="runtime-routing-list">{configurableTools.map((tool) => { const Icon = iconMap[tool.icon] || Wrench; const selectedConnection = data.connections.find((item) => item.id === tool.modelConnectionId); return <article key={tool.id}><span className={`tool-icon compact ${tool.category}`}><Icon size={18} /></span><div><strong>{isEn ? tool.nameEn : tool.nameZh}</strong><small>{t.currentModel}：{selectedConnection?.name || "OneShowModel"}</small></div><button onClick={() => openToolSettings(tool)}><GearSix size={14} />{t.toolSettings}</button></article>; })}</div>;
  return <div className="runtime-dashboard-layout"><main className="runtime-page">
    ...
    <article className="runtime-overview surface"><div className="runtime-overview-model"><span className="runtime-model-logo large"><img src="/brand/oneshowtools-mark.png" alt="" /></span><div><h2>{data.managed.name}<small>{isEn ? "Managed model" : "平台托管模型"}</small></h2><p>{t.managedDescription}</p><span className={data.managed.configured ? "runtime-connected" : "runtime-disconnected"}><span className={`runtime-live-dot ${data.managed.configured ? "on" : ""}`} />{data.managed.configured ? t.runtimeReady : t.notConfigured}</span></div></div><div className="runtime-overview-metrics">{runtimeMetrics.map(([label, value, Icon]) => <div key={label}><Icon size={18} weight="duotone" /><span>{label}</span><strong>{Number(value).toLocaleString()}</strong></div>)}</div></article>
    <nav className="runtime-tabs">{tabs.map(([id, label]) => <button className={runtimeTab === id ? "active" : ""} key={id} onClick={() => setRuntimeTab(id)}>{label}</button>)}</nav>
    {runtimeTab === "connections" && <section className="runtime-tab-grid"><div className="runtime-section"><div className="runtime-section-heading"><div><h2>{t.personalModels}</h2><p>{t.connectionsHint}</p></div><span>{healthyConnections.length}/{data.connections.length} {isEn ? "healthy" : "健康"}</span></div>{connectionRows}</div><aside className="runtime-routing-preview surface"><header><div><h2>{isEn ? "Default model routing" : "默认模型路由"}</h2><p>{isEn ? "Choose the model used by each AI tool." : "为每一个 AI 工具选择默认调用模型。"}</p></div><button onClick={() => setRuntimeTab("routing")}>{isEn ? "View all" : "查看全部"}<ArrowRight size={14} /></button></header>{configurableTools.slice(0, 6).map((tool) => { const selectedConnection = data.connections.find((item) => item.id === tool.modelConnectionId); return <button key={tool.id} onClick={() => openToolSettings(tool)}><span>{isEn ? tool.nameEn : tool.nameZh}</span><strong>{selectedConnection?.name || "OneShowModel"}</strong><GearSix size={14} /></button>; })}</aside></section>}
    {runtimeTab === "routing" && <section className="runtime-section"><div className="runtime-section-heading"><div><h2>{t.toolRouting}</h2><p>{t.toolRoutingHint}</p></div><span>{configurableTools.length} {isEn ? "configurable tools" : "个可配置工具"}</span></div>{routingRows}</section>}
    ...
    <article className="runtime-guide surface"><h2>{isEn ? "Quick setup guide" : "快速接入指南"}</h2>{[[isEn ? "Add a model connection" : "添加模型连接", isEn ? "Connect your API key and endpoint" : "连接你的 API Key 或自定义接口"],[isEn ? "Configure tool models" : "配置工具模型", isEn ? "Choose the right model for each tool" : "为每个工具选择合适的模型"],[isEn ? "Start using tools" : "开始使用", isEn ? "Run real AI tasks from the marketplace" : "从工具市场运行真实 AI 任务"]].map(([title, body], index) => <div key={title}><span>{index + 1}</span><p><strong>{title}</strong><small>{body}</small></p></div>)}<button onClick={() => setRuntimeTab("connections")}>{isEn ? "Open connection settings" : "进入连接设置"}<ArrowRight size={14} /></button></article>
    <article className="runtime-side-monitor surface"><header><h2>{isEn ? "Runtime monitor" : "运行监控"}</h2><span className={data.managed.configured ? "online" : "offline"}>{data.managed.configured ? t.online : t.notConfigured}</span></header>{runtimeMetrics.map(([label, value, Icon]) => <div key={label}><span><Icon size={16} />{label}</span><strong>{Number(value).toLocaleString()}</strong></div>)}</article>
...
function Billing({ plans, status, credits, user, tasks = [], locale, onCheckout, onPortal, onNavigate }) {
  ...
  const billingCopy = isEn ? {
    heading: "Billing center", headingSub: "Flexible credit packs and memberships for different stages of work.", overview: "Usage overview", monthSpent: "Spent this month", monthEarned: "Earned this month", entries: "Ledger entries", todaySpent: "Spent today", creditsTab: "Credit top-ups", memberTab: "Monthly membership", creditHint: "One-time purchase · Credits remain available for future tools and services", account: "Account information", manage: "Manage subscription", needMore: "Need more credits?", needMoreBody: "Top up credits to unlock more tools and services.", reasons: "Why choose credits?", durable: "Flexible balance", durableSub: "Use credits across all supported tools", secure: "Traceable & secure", secureSub: "Every charge is recorded in your ledger", value: "Better value", valueSub: "Higher packs include additional credits", recent: "Recent charges", noSpend: "No recent credit charges", details: "Credit ledger",
  } : {
    heading: "计费中心", headingSub: "灵活的计费方案，满足你的各种需求", overview: "本月使用概览", monthSpent: "本月消耗", monthEarned: "本月获得", entries: "流水笔数", todaySpent: "今日消耗", creditsTab: "积分充值", memberTab: "会员月付", creditHint: "一次性购买积分，长期有效，可用于所有已接入的工具和服务", account: "账户信息", manage: "管理订阅", needMore: "积分不够用？", needMoreBody: "充值积分，解锁更多强大工具和服务。", reasons: "为什么选择积分？", durable: "灵活通用", durableSub: "适用于所有已接入的工具和服务", secure: "安全可追踪", secureSub: "每次消耗都有清晰账本记录", value: "性价比高", valueSub: "更高档套餐包含额外赠送", recent: "最近消费", noSpend: "暂无积分消费记录", details: "积分明细",
  };
  return <div className="billing-dashboard-layout"><main className="billing-page">
    ...
    <section className="billing-overview surface"><article className="billing-credit-balance"><div><small>{t.creditsBalance}</small><strong>{(credits?.balance || 0).toLocaleString()}</strong><span>Credits</span></div><img src="/credits/credits-coin-stack.png" alt="" aria-hidden="true" /></article><div className="billing-overview-body"><header><strong>{billingCopy.overview}</strong><span>{monthStart.toLocaleDateString(isEn ? "en-US" : "zh-CN",{month:"2-digit",day:"2-digit"})} - {new Date().toLocaleDateString(isEn ? "en-US" : "zh-CN",{month:"2-digit",day:"2-digit"})}</span></header><div>{[[billingCopy.monthSpent,monthlySpent],[billingCopy.monthEarned,monthlyEarned],[billingCopy.entries,ledger.length],[billingCopy.todaySpent,todaySpent]].map(([label,value]) => <article key={label}><small>{label}</small><strong>{Number(value).toLocaleString()}</strong><span>{label === billingCopy.entries ? billingCopy.details : "Credits"}</span></article>)}</div></div><div className="billing-overview-actions"><button className="secondary-button" onClick={() => onNavigate?.("credits")}><Receipt size={16} />{billingCopy.details}</button><button className="primary-button" onClick={() => document.getElementById("billing-topups")?.scrollIntoView({behavior:"smooth"})}><Coins size={16} weight="fill" />{copy.topup}</button></div></section>
...
function TaskUsageDonut({ items }) {
  ...
  useEffect(() => {
    ...
    if (!items.length) { context.beginPath(); context.arc(size / 2,size / 2,31,0,Math.PI*2); context.strokeStyle="#e8edf5"; context.lineWidth=13; context.stroke(); }
  }, [items]);
...
function Tasks({ tasks, user, credits, billing, locale, onRefresh, onCancel, onOpenTask, onNavigate }) {
  ...
  const [page, setPage] = useState(1);
  const copy = isEn ? {
    subtitle: "Review every task, input, output and credit charge. Reopen completed work or cancel eligible jobs.", all: "All", running: "Running", completed: "Completed", failed: "Failed", cancelled: "Cancelled", total: "All tasks", todayCredits: "Credits spent today", taskInfo: "Task", tool: "Tool", status: "Status", cost: "Credits", created: "Created", action: "Action", allTools: "All tools", allTime: "All time", days7: "Last 7 days", days30: "Last 30 days", account: "Task account", currentPlan: "Current plan", usage: "Usage overview", quick: "Quick actions", newTask: "Create a task", newTaskSub: "Choose a ready tool", marketplace: "Tool marketplace", runtime: "AI Runtime", ledger: "Credit ledger", recent: "Recently used tools", noTasks: "No tasks match these filters.", prompt: "Input", page: "Page", topup: "Top up credits", viewAll: "View all",
  } : {
    subtitle: "查看所有任务的状态、输入和输出记录，支持重新打开结果并取消符合条件的任务。", all: "全部", running: "运行中", completed: "已完成", failed: "失败", cancelled: "已取消", total: "全部任务", todayCredits: "今日消耗积分", taskInfo: "任务信息", tool: "工具", status: "状态", cost: "消耗积分", created: "创建时间", action: "操作", allTools: "全部工具", allTime: "全部时间", days7: "最近 7 天", days30: "最近 30 天", account: "任务账户", currentPlan: "当前方案", usage: "任务使用概览", quick: "快捷操作", newTask: "创建新任务", newTaskSub: "快速开始使用 AI 工具", marketplace: "工具市场", runtime: "AI Runtime", ledger: "积分明细", recent: "最近使用工具", noTasks: "当前筛选条件下没有任务。", prompt: "输入", page: "页", topup: "充值积分", viewAll: "查看全部",
  };
  ...
  const todayStart = new Date(); todayStart.setHours(0,0,0,0);
  const todayCredits = tasks.filter((task) => task.createdAt >= todayStart.getTime()).reduce((sum,task) => sum + Number(task.creditCost || 0),0);
  ...
  const filtered = tasks.filter((task) => (statusFilter === "all" || normalizedStatus(task.status) === statusFilter) && (toolFilter === "all" || task.toolId === toolFilter) && task.createdAt >= cutoff);
  const pageSize = 8;
  const pageCount = Math.max(1,Math.ceil(filtered.length / pageSize));
  const safePage = Math.min(page,pageCount);
  const visible = filtered.slice((safePage-1)*pageSize,safePage*pageSize);
  const distributionMap = tasks.reduce((map,task) => map.set(task.toolNameZh,(map.get(task.toolNameZh)||0)+1),new Map());
  const colors = ["#6f6ae9","#52a7ec","#63cfad","#b190ea"];
  const distribution = [...distributionMap.entries()].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([name,value],index)=>({name,value,color:colors[index]}));
  const recentTools = [...new Map(tasks.map((task)=>[task.toolId,task])).values()].slice(0,4);
  const planName = billing?.subscription ? (isEn ? billing.subscription.nameEn : billing.subscription.nameZh) : t.free;
  const selectFilter = (setter) => (event) => { setter(event.target.value); setPage(1); };
  return <div className="tasks-dashboard-layout"><main className="tasks-page"><section className="tasks-heading"><div><h1>{t.nav.tasks}</h1><p>{copy.subtitle}</p></div><button className="secondary-button" onClick={onRefresh}><ArrowClockwise size={17} />{t.retry}</button></section>
    <section className="tasks-metrics">{[[copy.total,tasks.length,ListChecks,"all"],[copy.completed,counts.completed||0,CheckCircle,"completed"],[copy.running,counts.running||0,SpinnerGap,"running"],[copy.failed,counts.failed||0,XCircle,"failed"],[copy.todayCredits,todayCredits,Coins,"credits"]].map(([label,value,Icon,tone]) => <article className="surface" key={label}><div><small>{label}</small><strong>{Number(value).toLocaleString()}</strong></div><span className={tone}><Icon size={20} weight="duotone" /></span></article>)}</section>
    <section className="tasks-command"><nav>{[["all",copy.all],["running",copy.running],["completed",copy.completed],["failed",copy.failed],["cancelled",copy.cancelled]].map(([id,label]) => <button className={statusFilter===id?"active":""} key={id} onClick={()=>{setStatusFilter(id);setPage(1);}}>{label}</button>)}</nav><div><label><select value={toolFilter} onChange={selectFilter(setToolFilter)}><option value="all">{copy.allTools}</option>{tools.map((tool)=><option key={tool.id} value={tool.id}>{tool.name}</option>)}</select><CaretDown size={14} /></label><label><CalendarBlank size={14} /><select value={period} onChange={selectFilter(setPeriod)}><option value="all">{copy.allTime}</option><option value="7">{copy.days7}</option><option value="30">{copy.days30}</option></select></label></div></section>
    <section className="surface tasks-table"><header><span>{copy.taskInfo}</span><span>{copy.tool}</span><span>{copy.status}</span><span>{copy.cost}</span><span>{copy.created}</span><span>{copy.action}</span></header>{visible.map((task) => { const Icon=iconMap[task.icon]||Wrench; const input=String(task.input?.text || task.input?.prompt || task.input?.topic || "").trim(); return <article key={task.id} role="button" tabIndex="0" onClick={()=>onOpenTask(task)} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" ") onOpenTask(task);}}><span className="tool-icon compact"><Icon size={19} /></span><div><strong>{isEn?task.toolNameEn:task.toolNameZh}</strong><small>{input ? `${copy.prompt}: ${input}` : formatDate(task.createdAt,locale)}</small></div><span>{isEn?task.toolNameEn:task.toolNameZh}</span><StatusPill status={task.status} locale={locale}/><em>−{task.creditCost}</em><time>{formatDate(task.createdAt,locale)}</time><div className="tasks-row-actions"><button title={copy.action} onClick={(event)=>{event.stopPropagation();onOpenTask(task);}}><Eye size={16}/></button>{["queued","waiting_for_runtime"].includes(task.status) && <button title={copy.cancelled} onClick={(event)=>{event.stopPropagation();onCancel(task.id);}}><X size={16}/></button>}<ArrowRight size={15}/></div></article>;})}{!visible.length && <div className="tasks-empty"><ListChecks size={24}/><span>{copy.noTasks}</span></div>}<footer><button disabled={safePage<=1} onClick={()=>setPage((value)=>Math.max(1,value-1))}><ArrowLeft size={15}/></button><span>{safePage} / {pageCount} {copy.page}</span><button disabled={safePage>=pageCount} onClick={()=>setPage((value)=>Math.min(pageCount,value+1))}><ArrowRight size={15}/></button></footer></section>
...
function Files({ files, quota, user, billing, locale, onUpload, onDelete, onDeleteMany, onNavigate }) {
  const t = dictionary[locale];
  const isEn = locale === "en";
  ...
  const copy = isEn ? {
    badge:"File Center", subtitle:"Manage every uploaded and generated file in one secure, searchable workspace.", total:"All files", storage:"Storage used", month:"Uploaded this month", oss:"OSS objects", local:"Local objects", all:"All files", image:"Images", audio:"Audio", video:"Video", document:"Documents", archive:"Archives", other:"Other", search:"Search file names", newest:"Newest first", oldest:"Oldest first", largest:"Largest first", name:"File name", size:"Size", type:"Type", time:"Uploaded", source:"Source", action:"Action", uploaded:"Manual upload", storageTitle:"File allowance", storageBody:`Your current membership can retain up to ${quota?.limit || 100} private files.`, storageManage:"Storage settings", tools:"File tools", upload:"Upload a file", uploadSub:"Up to 25 MB per file", tasks:"Task outputs", tasksSub:"Files created by your AI tools", privacy:"Private storage", privacySub:"Only your account can access files", formats:"Supported file types", formatsBody:"Images, audio, video, documents, PDFs and archives", empty:"No files match these filters.", page:"Page", plan:"Plan", download:"Download", remove:"Delete", list:"List view", grid:"Grid view", selectAll:"Select current page", selected:"selected", clear:"Clear selection", deleteSelected:"Delete selected", confirmDelete:"Delete the selected files?", confirmBody:"This permanently removes the files and cannot be undone.", cancel:"Cancel", confirm:"Delete files", deleting:"Deleting...",
  } : {
    badge:"File Center", subtitle:"集中管理所有上传与生成文件，支持安全存储、分类查找和便捷下载。", total:"全部文件", storage:"文件大小", month:"本月上传", oss:"OSS 文件", local:"本地文件", all:"全部文件", image:"图片", audio:"音频", video:"视频", document:"文档", archive:"压缩包", other:"其他", search:"搜索文件名", newest:"按更新时间", oldest:"按最早时间", largest:"按文件大小", name:"文件名", size:"大小", type:"类型", time:"上传时间", source:"来源", action:"操作", uploaded:"手动上传", storageTitle:"文件额度", storageBody:`当前会员最多可保留 ${quota?.limit || 100} 个私有文件。`, storageManage:"存储设置", tools:"文件工具", upload:"上传文件", uploadSub:"单个文件最大 25 MB", tasks:"任务产物", tasksSub:"由 AI 工具生成并保存的文件", privacy:"私有存储", privacySub:"仅当前账户可以访问", formats:"支持的文件类型", formatsBody:"图片、音频、视频、文档、PDF 与压缩包", empty:"当前筛选条件下没有文件。", page:"页", plan:"当前方案", download:"下载", remove:"删除", list:"列表视图", grid:"网格视图", selectAll:"全选当前页", selected:"个文件已选择", clear:"取消选择", deleteSelected:"删除所选", confirmDelete:"确定删除所选文件？", confirmBody:"文件将被永久删除，此操作无法撤销。", cancel:"取消", confirm:"确认删除", deleting:"正在删除...",
  };
  ...
  const filtered = files.filter((file)=>(category==="all"||fileType(file.mimeType,file.name)===category)&&file.name.toLowerCase().includes(search.trim().toLowerCase())).sort((a,b)=>sort==="oldest"?a.createdAt-b.createdAt:sort==="largest"?b.sizeBytes-a.sizeBytes:b.createdAt-a.createdAt);
  ...
  return <div className="files-dashboard-layout"><main className="files-page"><section className="files-heading"><div><h1>{t.nav.files}<span>{copy.badge}</span></h1><p>{copy.subtitle}</p></div><input ref={inputRef} className="visually-hidden" type="file" onChange={(event)=>{const file=event.target.files?.[0];if(file)onUpload(file);event.target.value="";}}/><button className="primary-button" onClick={chooseFile}><CloudArrowUp size={17}/>{t.upload}</button></section>
    <section className="files-metrics">{[[copy.total,files.length,FolderOpen,"total"],[copy.storage,formatBytes(totalBytes),Database,"storage"],[copy.month,files.filter((file)=>file.createdAt>=monthStart).length,ArrowUp,"month"],[copy.oss,files.filter((file)=>file.storageProvider==="oss").length,CloudArrowUp,"oss"],[copy.local,files.filter((file)=>file.storageProvider!=="oss").length,HardDrives,"local"]].map(([label,value,Icon,tone])=><article className="surface" key={label}><div><small>{label}</small><strong>{typeof value==="number"?value.toLocaleString():value}</strong></div><span className={tone}><Icon size={20} weight="duotone"/></span></article>)}</section>
    <section className="surface files-browser"><header><nav>{categoryTabs.map(([id,label])=><button key={id} className={category===id?"active":""} onClick={()=>{setCategory(id);setPage(1);}}>{label}{id!=="all"&&categoryCounts[id]?<small>{categoryCounts[id]}</small>:null}</button>)}</nav><div><label className="files-search"><MagnifyingGlass size={15}/><input value={search} onChange={(event)=>{setSearch(event.target.value);setPage(1);}} placeholder={copy.search}/></label><label className="files-sort"><select value={sort} onChange={(event)=>{setSort(event.target.value);setPage(1);}}><option value="newest">{copy.newest}</option><option value="oldest">{copy.oldest}</option><option value="largest">{copy.largest}</option></select><CaretDown size={13}/></label><label className="files-select-visible"><input type="checkbox" checked={allVisibleSelected} onChange={toggleVisible}/><span/>{copy.selectAll}</label><div className="files-layout-toggle"><button className={layout==="list"?"active":""} onClick={()=>setLayout("list")} title={copy.list}><ListChecks size={16}/></button><button className={layout==="grid"?"active":""} onClick={()=>setLayout("grid")} title={copy.grid}><GridFour size={16}/></button></div></div></header>
      ...
      {layout==="list"?<div className="files-list"><div className="files-list-head"><label className="file-checkbox"><input type="checkbox" checked={allVisibleSelected} onChange={toggleVisible} aria-label={copy.selectAll}/><span/></label><span>{copy.name}</span><span>{copy.size}</span><span>{copy.type}</span><span>{copy.time}</span><span>{copy.source}</span><span>{copy.action}</span></div>{visible.map((file)=>{const [label,Icon,tone]=typeMeta[fileType(file.mimeType,file.name)];return <article className={selectedIds.has(file.id)?"selected":""} key={file.id}><label className="file-checkbox"><input type="checkbox" checked={selectedIds.has(file.id)} onChange={()=>toggleFile(file.id)} aria-label={`${copy.name}: ${file.name}`}/><span/></label><span className={`files-type-icon ${tone}`}><Icon size={18} weight="duotone"/></span><strong title={file.name}>{file.name}</strong><span>{formatBytes(file.sizeBytes)}</span><span className={`files-type-pill ${tone}`}>{label}</span><time>{formatDate(file.createdAt,locale)}</time><span>{(isEn?file.sourceNameEn:file.sourceNameZh)||copy.uploaded}</span><div><a href={`/api/files/${file.id}/download`} title={copy.download}><DownloadSimple size={16}/></a><button onClick={()=>onDelete(file.id)} title={copy.remove}><Trash size={16}/></button></div></article>})}</div>:<div className="files-grid">{visible.map((file)=>{const [label,Icon,tone]=typeMeta[fileType(file.mimeType,file.name)];return <article className={selectedIds.has(file.id)?"selected":""} key={file.id}><label className="file-checkbox grid"><input type="checkbox" checked={selectedIds.has(file.id)} onChange={()=>toggleFile(file.id)} aria-label={`${copy.name}: ${file.name}`}/><span/></label><span className={`files-type-icon ${tone}`}><Icon size={24} weight="duotone"/></span><div><strong title={file.name}>{file.name}</strong><small>{label} · {formatBytes(file.sizeBytes)}</small></div><footer><time>{formatDate(file.createdAt,locale)}</time><a href={`/api/files/${file.id}/download`} title={copy.download}><DownloadSimple size={16}/></a><button onClick={()=>onDelete(file.id)} title={copy.remove}><Trash size={16}/></button></footer></article>})}</div>}
...
function Account({ user, health, credits, billing, locale, onLogout, onUserChange, onLocaleChange, onNotice, onNavigate }) {
  ...
  const copy = isEn ? {
    ...
    title:"用户系统", subtitle:"管理你的账户资料、安全设置、登录会话和账户数据。", profile:"账户设置", security:"安全设置", sessions:"登录会话", privacy:"数据与注销", overview:"账户概览", plan:"会员等级", expires:"加入时间", balance:"可用积分", spent:"总消耗积分", month:"本月消耗", profileTitle:"账户信息", name:"姓名", email:"邮箱地址", language:"语言偏好", save:"保存修改", verified:"已验证", pending:"待验证", securityTitle:"安全信息", password:"登录密码", passwordHint:"建议使用至少 10 位且不与其他网站重复的密码。", changePassword:"修改密码", currentPassword:"当前密码", newPassword:"新密码", emailTitle:"更换邮箱", emailHint:"新邮箱会收到验证链接，验证后才会正式生效。", newEmail:"新邮箱地址", sendVerification:"发送验证邮件", sessionTitle:"登录会话", sessionHint:"查看当前正在使用你账户的设备，并可随时下线。", current:"当前登录", browser:"浏览器会话", lastActive:"最近活跃", expiresAt:"有效期至", revoke:"下线", revokeOthers:"下线其他设备", dataTitle:"账户数据", export:"导出账户数据", exportHint:"下载包含资料、积分、任务和文件清单的 JSON 数据包。", exportAction:"创建并下载", deleteTitle:"注销账户", deleteHint:"提交后进入 7 天安全等待期，避免误操作。", deleteAction:"申请注销", unavailable:"当前部署未开放账户注销能力。", quick:"快速操作", runtime:"管理模型与 API", credits:"查看积分明细", billing:"计费与会员方案", logout:"退出登录", help:"账户安全说明", helpText:"更换邮箱需要密码确认，所有登录会话都可独立下线。", free:"免费版", status:"账户状态", active:"正常", joined:"加入时间", noSessions:"暂无有效登录会话。", api:"AI Runtime",
  };
  const ledger = credits?.ledger || [];
  ...
  return <div className="account-dashboard-layout"><main className="account-page"><section className="account-heading"><div><h1>{copy.title}</h1><p>{copy.subtitle}</p></div></section><nav className="account-tabs">{[["profile",User,copy.profile],["security",ShieldCheck,copy.security],["sessions",HardDrives,copy.sessions],["privacy",LockKey,copy.privacy],["runtime",PlugsConnected,copy.api]].map(([id,Icon,label])=><button key={id} className={section===id?"active":""} onClick={()=>id==="runtime"?onNavigate?.("runtime"):setSection(id)}><Icon size={16}/>{label}</button>)}</nav>
    <div className="account-body"><article className="surface account-profile-card"><div className="account-profile-glow"/><span className="account-avatar">{user.name.slice(0,1).toUpperCase()}</span><h2>{user.name}</h2><p>{user.email}</p><strong><Crown size={14} weight="fill"/>{planName}</strong><dl><div><dt>{copy.expires}</dt><dd>{formatDate(user.createdAt,locale)}</dd></div><div><dt>{copy.balance}</dt><dd>{Number(credits?.balance||0).toLocaleString()} Credits</dd></div></dl><button onClick={()=>setSection("profile")}><NotePencil size={16}/>{copy.profile}</button></article>
      <div className="account-center">
        {section==="profile"&&<><article className="surface account-panel"><header><div><h2>{copy.profileTitle}</h2><p>{copy.subtitle}</p></div><span className={`account-status ${user.emailVerified?"verified":"pending"}`}><CheckCircle size={14} weight="fill"/>{user.emailVerified?copy.verified:copy.pending}</span></header><form className="account-profile-form" onSubmit={saveProfile}><label>{copy.name}<input required maxLength={80} value={profile.name} onChange={(event)=>setProfile({...profile,name:event.target.value})}/></label><label>{copy.email}<div className="account-readonly"><span>{user.email}</span><LockKey size={14}/></div></label><label className="wide">{copy.language}<select value={profile.locale} onChange={(event)=>setProfile({...profile,locale:event.target.value})}><option value="zh-CN">简体中文</option><option value="en">English</option></select></label><button className="primary-button wide" type="submit">{copy.save}</button></form></article><article className="surface account-panel account-summary-list"><header><div><h2>{copy.securityTitle}</h2><p>{copy.passwordHint}</p></div><button className="text-button" onClick={()=>setSection("security")}>{copy.changePassword}<ArrowRight size={13}/></button></header><div><span><LockKey size={16}/>{copy.password}</span><strong>••••••••••••</strong></div><div><span><ShieldCheck size={16}/>{copy.email}</span><strong className="verified"><CheckCircle size={13} weight="fill"/>{user.emailVerified?copy.verified:copy.pending}</strong></div></article><article className="surface account-panel account-summary-list"><header><div><h2>{copy.sessionTitle}</h2><p>{copy.sessionHint}</p></div><button className="text-button" onClick={()=>setSection("sessions")}>{copy.sessions}<ArrowRight size={13}/></button></header>{sessions.slice(0,2).map((session)=><div key={session.id}><span><HardDrives size={16}/>{sessionName(session.userAgent)}</span><strong>{session.current?copy.current:formatDate(session.lastSeenAt||session.createdAt,locale)}</strong></div>)}</article></>}
    ...
    </div></main><aside className="account-side"><article className="surface account-overview"><h2>{copy.overview}</h2>{[[copy.plan,planName,Crown],[copy.status,copy.active,CheckCircle],[copy.balance,`${Number(credits?.balance||0).toLocaleString()} Credits`,Coins],[copy.spent,`${totalSpent.toLocaleString()} Credits`,ArrowDown],[copy.month,`${monthSpent.toLocaleString()} Credits`,ChartLineUp]].map(([label,value,Icon])=><div key={label}><span><Icon size={15}/>{label}</span><strong>{value}</strong></div>)}</article><article className="surface account-help"><h2>{copy.help}</h2><span><ShieldCheck size={22} weight="duotone"/></span><p>{copy.helpText}</p></article><article className="surface account-quick"><h2>{copy.quick}</h2>{[[PlugsConnected,copy.runtime,"runtime"],[Coins,copy.credits,"credits"],[CreditCard,copy.billing,"billing"]].map(([Icon,label,target])=><button key={target} onClick={()=>onNavigate?.(target)}><span><Icon size={16}/></span><strong>{label}</strong><ArrowRight size={14}/></button>)}<button className="logout" onClick={onLogout}><span><SignOut size={16}/></span><strong>{copy.logout}</strong><ArrowRight size={14}/></button></article></aside></div>;
}
...
export function App() {
  ...
  const [view, setView] = useState(() => {
    ...
    return ["dashboard", "marketplace", "runtime", "credits", "billing", "tasks", "files", "account"].includes(requestedView) ? requestedView : "dashboard";
  });
  ...
  const content = {
    dashboard: <Dashboard data={privateData.dashboard} tools={tools} runtime={privateData.runtime} locale={locale} onNavigate={setView} onSearch={(value) => { setQuery(value); setView("marketplace"); }} onRun={openTool} />,
    marketplace: <Marketplace tools={tools} locale={locale} query={query} onQuery={setQuery} onRun={openTool} data={privateData.dashboard} runtime={privateData.runtime} tasks={privateData.tasks} onNavigate={setView} />,
    ...
    files: <Files files={privateData.files} quota={privateData.fileQuota} user={session} billing={privateData.billing} locale={locale} onUpload={upload} onDelete={deleteFile} onDeleteMany={deleteFiles} onNavigate={setView} />,
  ...
  const usesFullWorkspace = view === "marketplace" || view === "runtime" || view === "credits" || view === "billing" || isToolWorkspace;
  return <div className="platform-shell"><aside className="sidebar"><Brand /><nav>{navItems.map(([key, Icon]) => <button className={view === key ? "active" : ""} onClick={() => navigateView(key)} key={key}><Icon size={20} weight={view === key ? "fill" : "regular"} /><span>{t.nav[key]}</span></button>)}</nav><SupportWidget locale={locale} /><div className="sidebar-footer"><div className="mini-profile"><span>{session.name.slice(0, 1).toUpperCase()}</span><div><strong>{session.name}</strong><small>{session.email}</small></div></div></div></aside>
