...
const aiImageToolSlugs = new Set(["ai-outfit-changer", "ai-id-photo", "ai-id-photo-studio", "ai-professional-headshot", "ai-product-photo", "ai-portrait-studio", "ai-smart-cutout", "ai-background-replacer", "ai-image-restorer", "sliding-ancestor-generator"]);
...
const imageToolFields = {
  ...
  "ai-id-photo-studio": [{ id: "mode", type: "select", zh: "生成类型", en: "Generation type", options: [["id-photo", "AI 证件照", "AI ID photo"], ["professional", "AI 职业照", "Professional headshot"], ["badge", "AI 工牌头像", "Employee badge portrait"]] }, { id: "backgroundColor", type: "select", zh: "背景颜色", en: "Background", options: [["white", "白色", "White"], ["blue", "蓝色", "Blue"], ["red", "红色", "Red"], ["light gray", "浅灰色", "Light gray"]] }, { id: "outfit", type: "select", zh: "服装", en: "Outfit", options: [["formal dark suit and white shirt", "深色正装 + 白衬衫", "Dark suit + white shirt"], ["white collared shirt", "白色衬衫", "White shirt"], ["preserve original clothing", "保留原服装", "Preserve original outfit"]] }, { id: "prompt", type: "textarea", zh: "工牌信息 / 补充要求（可选）", en: "Badge details / additional direction (optional)", placeholderZh: "例如：姓名、职位、公司名称；Beta 版先生成头像预览", placeholderEn: "e.g. name, role, company; Beta currently previews the portrait" }],
...
const dictionary = {
  "zh-CN": {
    nav: { dashboard: "工作台", marketplace: "AI 工具市场", recent: "最近使用", favorites: "我的收藏", agent: "AI Agent", runtime: "AI Runtime", downloads: "下载客户端", tasks: "任务中心", files: "文件中心", projects: "项目中心", plans: "积分与套餐", billing: "计费中心", settings: "设置中心", credits: "积分", account: "用户系统" },
  ...
  en: {
    nav: { dashboard: "Workspace", marketplace: "AI Marketplace", recent: "Recently used", favorites: "Favorites", agent: "AI Agent", runtime: "AI Runtime", downloads: "Desktop app", tasks: "Task Center", files: "File Center", projects: "Project Center", plans: "Credits & Plans", billing: "Billing", settings: "Settings", credits: "Credits", account: "Account" },
...
const dashboardRecommendedSlugs = ["ai-outfit-changer", "food-nutrition-analyzer", "ai-fridge-recipe", "ai-music-studio"];
const dashboardPopularSlugs = ["ai-music-studio", "ai-outfit-changer", "food-nutrition-analyzer", "ai-fridge-recipe", "sliding-ancestor-generator", "mbti-personality-test"];
// The public catalog does not expose a publishedAt field yet. Keep this order in
// one explicit merchandising config until the admin API can provide it.
const dashboardNewSlugs = ["sliding-ancestor-generator", "mbti-personality-test", "hang-la-tier-list-generator", "stock-pet"];

function dashboardToolSelection(tools, slugs, limit) {
  const catalog = Array.isArray(tools) ? tools : [];
  return [...slugs.map((slug) => catalog.find((tool) => tool.slug === slug)).filter(Boolean), ...catalog]
    .filter((tool, index, list) => list.findIndex((item) => item.id === tool.id) === index)
    .slice(0, limit);
}

function DashboardSectionHeader({ icon: Icon, title, action, onAction }) {
  return <header className="dashboard-section-heading"><div>{Icon && <Icon size={18} weight="duotone" />}<h2>{title}</h2></div>{action && <button type="button" onClick={onAction}>{action}<ArrowRight size={14} /></button>}</header>;
}

function DashboardToolCard({ tool, locale, tone, onRun }) {
  const isEn = locale === "en";
  return <button type="button" className={`dashboard-tool-card ${tone || ""}`} onClick={() => onRun(tool)}>
    <span className="dashboard-tool-visual"><ProductToolIcon tool={tool} size={34} /></span>
    <span className="dashboard-tool-copy"><strong>{isEn ? tool.nameEn : tool.nameZh}</strong><small>{isEn ? tool.descriptionEn : tool.descriptionZh}</small><em>{isEn ? "Use now" : "立即使用"}<ArrowRight size={13} /></em></span>
  </button>;
}

function DashboardShortcut({ tool, locale, onRun, badge }) {
  const isEn = locale === "en";
  return <button type="button" className="dashboard-shortcut" onClick={() => onRun(tool)}>
    <ProductToolIcon tool={tool} size={24} />
    <span><strong>{isEn ? tool.nameEn : tool.nameZh}</strong>{badge && <small>{badge}</small>}</span>
  </button>;
}
...
function Dashboard({ data, tools, tasks = [], runtime, locale, onNavigate, onSearch, onRun, onOpenTask }) {
  ...
  const hasPaidPlan = Boolean(data.subscription && data.subscription.code !== "free");
  const recommended = dashboardToolSelection(tools, dashboardRecommendedSlugs, 4);
  ...
  ].map(([slug, label, query]) => ({ slug, label, query, tool: tools.find((item) => item.slug === slug) }));
  const submitSearch = (event) => {
    event.preventDefault();
    onSearch(homeQuery.trim());
  };
  return <div className="page-stack dashboard-page dashboard-home-v4">
    ...
    <section className="dashboard-status-bar">
      {[[Coins, data.metrics.credits, isEn ? "Available credits" : "可用积分", "gold"], [Crown, currentPlan, isEn ? "Current plan" : "当前套餐", "violet"], [CheckCircle, data.metrics.completed, isEn ? "Completed tasks" : "已完成任务", "orange"], [FolderOpen, data.metrics.files, isEn ? "Private files" : "私有文件", "green"]].map(([Icon, value, label, tone]) => <article key={label}><span className={`dashboard-status-icon ${tone}`}><Icon size={22} weight="duotone" /></span><div><strong>{typeof value === "number" ? value.toLocaleString() : value}</strong><small>{label}</small></div></article>)}
      <button type="button" onClick={() => onNavigate("plans")}><Coins size={17} />{isEn ? "Recharge credits" : "积分充值"}<ArrowRight size={15} /></button>
    </section>

    <section className="dashboard-v4-layout">
      <div className="dashboard-content-main">
        <div className="dashboard-feature-row">
          <article className="surface dashboard-continue">
            <DashboardSectionHeader icon={ArrowsClockwise} title={isEn ? "Continue creating" : "继续创作"} />
            {continueTask ? <div className="dashboard-continue-body"><div className="dashboard-continue-title"><ProductToolIcon tool={continueTool || { icon: continueTask.icon }} size={27} /><span><small>{isEn ? continueTask.toolNameEn : continueTask.toolNameZh}</small><strong>{taskTitle || (isEn ? continueTask.toolNameEn : continueTask.toolNameZh)}</strong></span></div><div className="dashboard-waveform" aria-hidden="true">{Array.from({ length: 22 }, (_, index) => <i key={index} style={{ height: `${8 + ((index * 13) % 23)}px` }} />)}</div><small>{isEn ? "Last edited" : "上次编辑"}：{formatDate(continueTask.updatedAt || continueTask.createdAt, locale)}</small><button type="button" onClick={() => onOpenTask ? onOpenTask(continueTask) : (continueTool && onRun(continueTool))}>{isEn ? "Continue" : "继续创作"}<ArrowRight size={16} /></button></div> : <div className="dashboard-empty"><ListChecks size={26} weight="duotone" /><strong>{isEn ? "No recent creation" : "还没有最近创作"}</strong><small>{isEn ? "Choose a tool to begin your first task." : "选择一个工具，开始你的第一个任务。"}</small><button type="button" onClick={() => onNavigate("marketplace")}>{isEn ? "Browse tools" : "浏览工具"}</button></div>}
          </article>
          <article className="surface dashboard-recommended-v4">
            <DashboardSectionHeader icon={Sparkle} title={isEn ? "Recommended for you" : "为你推荐"} action={isEn ? "More tools" : "更多工具"} onAction={() => onNavigate("marketplace")} />
            <div>{recommended.map((tool, index) => <DashboardToolCard key={tool.id} tool={tool} locale={locale} tone={`tone-${index + 1}`} onRun={onRun} />)}</div>
          </article>
        </div>
        <div className="dashboard-discovery-row">
          <article className="surface dashboard-shortcut-panel"><DashboardSectionHeader icon={Fire} title={isEn ? "Popular tools" : "热门工具"} action={isEn ? "View more" : "查看更多"} onAction={() => onNavigate("marketplace")} /><div>{popular.map((tool) => <DashboardShortcut key={tool.id} tool={tool} locale={locale} onRun={onRun} />)}</div></article>
          <article className="surface dashboard-shortcut-panel"><DashboardSectionHeader icon={Sparkle} title={isEn ? "New this week" : "本周新上线"} action={isEn ? "View more" : "查看更多"} onAction={() => onNavigate("marketplace")} /><div>{newTools.map((tool) => <DashboardShortcut key={tool.id} tool={tool} locale={locale} onRun={onRun} badge={tool.slug === "sliding-ancestor-generator" ? (isEn ? "NEW" : "新") : ""} />)}</div></article>
        </div>
      </div>
    ...
    <section className="dashboard-pro-banner">
      <div><small>{hasPaidPlan ? (isEn ? "Your professional workspace" : "你的专业创作空间") : (isEn ? "Unlock the complete workspace" : "解锁完整创作能力")}</small><h2>{hasPaidPlan ? (isEn ? "Make more with OneShowTools Pro" : "使用 OneShowTools 专业版高效创作") : (isEn ? "Upgrade to Pro" : "升级专业版，解锁全部 AI 能力")}</h2><p>{isEn ? "Faster generation, advanced models and more commercial-ready tools in one workspace." : "更快生成、进阶模型与更多商业级工具，都在一个工作台完成。"}</p><button type="button" onClick={() => onNavigate("plans")}>{hasPaidPlan ? (isEn ? "Manage plan" : "管理套餐") : (isEn ? "Upgrade now" : "升级专业版")}<ArrowRight size={15} /></button></div>
      <div className="dashboard-pro-benefits">{[[SquaresFour, isEn ? "All tools" : "更多工具"], [Lightning, isEn ? "Priority generation" : "优先生成"], [Brain, isEn ? "Advanced models" : "高级模型"], [ShieldCheck, isEn ? "Commercial workspace" : "商业权益"]].map(([Icon, label]) => <span key={label}><Icon size={20} weight="duotone" /><small>{label}</small></span>)}</div><Crown className="dashboard-pro-crown" size={96} weight="duotone" aria-hidden="true" />
    </section>
  </div>;
}
...
function Marketplace({ tools, locale, query, onQuery, onRun, data, runtime, tasks = [], onNavigate, favorites = [], onToggleFavorite = () => {} }) {
  ...
  const carouselItems = tools.filter((tool) => Number(tool.featuredRank) > 0)
  ...
  return <div className="marketplace-page marketplace-page-redesign">
    <div className="marketplace-primary">
      ...
      {carouselItems.length > 0 && <section className="marketplace-featured surface">
      ...
      <section className="marketplace-catalog surface">
        ...
        <div className="marketplace-catalog-body">
          ...
          <div className="marketplace-card-area">
            ...
            {visible.length ? <div className="marketplace-tool-grid">{visible.map((tool) => { const ready = tool.runtimeStatus === "ready"; const favorite = favorites.includes(tool.id); return <article className="marketplace-tool-card" key={tool.id}><div className="tool-card-head"><ProductToolIcon tool={tool} size={22} /><div>{tool.publicationState === "testing" && <span className="tool-testing-badge">{isEn ? "ADMIN TEST" : "管理员测试"}</span>}<button className={`tool-favorite ${favorite ? "active" : ""}`} aria-label={isEn ? "Toggle favorite" : "切换收藏"} onClick={() => onToggleFavorite(tool.id)}><Star size={15} weight={favorite ? "fill" : "regular"} /></button>{ready ? <span className="tool-ready"><CheckCircle size={13} weight="fill" />{t.ready}</span> : <StatusPill status={tool.runtimeStatus} locale={locale} />}</div></div><h3>{locale === "en" ? tool.nameEn : tool.nameZh}</h3><p>{locale === "en" ? tool.descriptionEn : tool.descriptionZh}</p><div className="tool-card-tags"><span>{isEn ? "Tool" : "工具"}</span><span>{t[tool.category] || tool.category}</span></div><footer><ToolPrice tool={tool} locale={locale} /><button onClick={() => onRun(tool)}>{isEn ? "Use" : "使用"}</button></footer></article>; })}</div> : <EmptyState icon={selectedCategory.icon} title={query ? t.noResults : t.comingSoon} body={query ? undefined : t.comingSoonHint} action={!query && <button className="secondary-button" onClick={() => setCategory("all")}>{t.all}</button>} />}
...
function Runtime({ data, dashboard, tasks = [], locale, onRefresh, onNotice, onNavigate, initialTab = "connections" }) {
  const t = dictionary[locale];
  ...
  const [runtimeTab, setRuntimeTab] = useState(initialTab);
  const [desktopReleaseDialog, setDesktopReleaseDialog] = useState(null);
  ...
  useEffect(() => {
    if (!showForm && !settingsTool && !desktopReleaseDialog) return undefined;
    const closeOnEscape = (event) => {
      if (event.key !== "Escape") return;
      setShowForm(false);
      setSettingsTool(null);
      setDesktopReleaseDialog(null);
    };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [showForm, settingsTool, desktopReleaseDialog]);
  if (!data) return <Loading locale={locale} />;
  ...
  const tabs = [
    ...
    ["desktop", isEn ? "Desktop app" : "桌面客户端"],
    ["monitoring", isEn ? "Runtime monitor" : "运行监控"],
  ];
  const switchRuntimeTab = (tab) => {
    if (tab === "desktop" && initialTab !== "desktop") {
      setRuntimeTab("desktop");
  ...
  const openDesktopRelease = (platform = "all") => setDesktopReleaseDialog(platform);
  const copyDesktopContact = async (value, label) => {
    try {
      await navigator.clipboard.writeText(value);
      onNotice(isEn ? `${label} copied.` : `已复制${label}。`);
    } catch {
      onNotice(value);
    }
  };
  ...
  return <div className="runtime-dashboard-layout runtime-v3"><main className="runtime-page">
    ...
    <section className="runtime-console surface"><nav className="runtime-tabs">{tabs.map(([id, label]) => <button className={runtimeTab === id ? "active" : ""} key={id} onClick={() => switchRuntimeTab(id)}>{label}{id === "desktop" && <em>NEW</em>}</button>)}</nav>
    ...
    {runtimeTab === "desktop" && <section className="runtime-section desktop-client-section">
      <section className="desktop-client-hero">
        <div className="desktop-client-hero-copy"><span className="desktop-client-eyebrow"><Desktop size={15} weight="duotone" />{isEn ? "ONSHOWTOOLS DESKTOP" : "ONSHOWTOOLS 桌面端"}</span><h2>{isEn ? "Your AI workspace, always one shortcut away" : "让 AI 工作台，真正成为桌面生产力"}</h2><p>{isEn ? "Launch your tools instantly, hand off local files and keep long-running tasks moving in the background — all with the same OneShowTools account." : "无需反复打开浏览器。随时唤起工具、直接处理本地文件，让耗时任务在后台继续运行，并与网页端账号实时同步。"}</p><div className="desktop-client-benefits">{[isEn ? "One account, every workspace" : "同一账号，多端同步", isEn ? "Background tasks stay active" : "任务后台持续运行", isEn ? "Local files, faster handoff" : "本地文件快速处理"].map((item) => <span key={item}><CheckCircle size={15} weight="fill" />{item}</span>)}</div><div className="desktop-client-hero-actions"><button type="button" onClick={() => openDesktopRelease("all")}><Bell size={17} weight="fill" />{isEn ? "View release status" : "查看发布状态"}</button><button type="button" className="desktop-hero-link" onClick={() => openDesktopRelease("contact")}>{isEn ? "Contact the product team" : "联系产品团队"}<ArrowRight size={14} /></button></div></div>
        <div className="desktop-client-hero-visual"><img src="/runtime/oneshow-runtime-platform.webp" alt="OneShowTools desktop runtime experience" /><div className="desktop-client-platform-float macos"><AppleLogo size={22} weight="fill" /><span><strong>macOS</strong><small>Apple Silicon / Intel</small></span></div><div className="desktop-client-platform-float windows"><WindowsLogo size={22} weight="fill" /><span><strong>Windows</strong><small>Windows 10 / 11</small></span></div></div>
      </section>
      <section className="desktop-platform-panel"><header><div><span>{isEn ? "CHOOSE YOUR PLATFORM" : "选择你的平台"}</span><h3>{isEn ? "Desktop clients in active development" : "桌面客户端正在开发中"}</h3></div><span className="desktop-client-status"><i />{isEn ? "Release preview" : "发布预告"}</span></header><div className="desktop-platform-grid">
        <article className="macos"><span className="desktop-platform-icon"><AppleLogo size={28} weight="fill" /></span><div><small>{isEn ? "DESKTOP APP" : "桌面客户端"}</small><h4>OneShowTools for macOS</h4><p>{isEn ? "Native support is planned for both Apple Silicon and Intel Macs." : "面向 Mac 的桌面工作台，计划同时支持 Apple Silicon 与 Intel 芯片。"}</p><ul><li>macOS 13.0+</li><li>Apple Silicon</li><li>Intel</li></ul></div><button type="button" onClick={() => openDesktopRelease("macos")}>{isEn ? "View macOS status" : "查看 macOS 发布状态"}<ArrowRight size={15} /></button></article>
        <article className="windows"><span className="desktop-platform-icon"><WindowsLogo size={28} weight="fill" /></span><div><small>{isEn ? "DESKTOP APP" : "桌面客户端"}</small><h4>OneShowTools for Windows</h4><p>{isEn ? "A focused Windows workspace for AI tools, local files and background tasks." : "面向 Windows 的原生体验，集中使用 AI 工具、本地文件与后台任务。"}</p><ul><li>Windows 10 / 11</li><li>64-bit</li></ul></div><button type="button" onClick={() => openDesktopRelease("windows")}>{isEn ? "View Windows status" : "查看 Windows 发布状态"}<ArrowRight size={15} /></button></article>
      </div></section>
      <section className="desktop-client-feature-block"><header><span>{isEn ? "BUILT FOR FLOW" : "为高效工作而生"}</span><h3>{isEn ? "Less switching. More finished work." : "减少切换，把时间留给真正的创作"}</h3></header><div>{[[Keyboard, isEn ? "Global shortcut" : "全局快捷唤起", isEn ? "Open your AI tools without breaking focus." : "不离开当前工作，也能快速调用常用工具。"],[Crop, isEn ? "Screenshot to AI" : "截图直接交给 AI", isEn ? "Capture any region and send it directly into a task." : "截取任意区域，直接进入 AI 分析与处理流程。"],[FolderOpen, isEn ? "Local file handoff" : "本地文件无缝处理", isEn ? "Drag files into the workspace without extra uploads." : "拖入本地文件即可开始处理，减少重复上传步骤。"],[Bell, isEn ? "Reliable notifications" : "任务完成及时提醒", isEn ? "Get notified when long-running tasks are ready." : "耗时任务在后台执行，完成后第一时间通知。"]].map(([Icon, title, body]) => <article key={title}><span><Icon size={22} weight="duotone" /></span><div><strong>{title}</strong><small>{body}</small></div></article>)}</div></section>
      <footer className="desktop-client-help"><ShieldCheck size={16} weight="fill" /><span>{isEn ? "No installer is available yet. Official packages will only be distributed through OneShowTools." : "客户端尚未正式发布。后续安装包将仅通过 OneShowTools 官方渠道提供。"}</span><a href="mailto:contact@oneshowailab.com">{isEn ? "Contact support" : "联系支持"}<ArrowRight size={13} /></a></footer>
    </section>}
    ...
    {runtimeTab !== "desktop" && <div className="runtime-security-note"><ShieldCheck size={18} weight="fill" /><span>{t.keyPrivacy}</span></div>}</section>
    {desktopReleaseDialog && <div className="runtime-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDesktopReleaseDialog(null); }}><div className="runtime-dialog desktop-release-dialog surface" role="dialog" aria-modal="true" aria-labelledby="desktop-release-dialog-title"><header><div><span className="runtime-dialog-icon"><Desktop size={21} /></span><div><h2 id="desktop-release-dialog-title">{desktopReleaseDialog === "contact" ? (isEn ? "Contact the product team" : "联系产品团队") : (isEn ? "Desktop client release status" : "桌面客户端发布状态")}</h2><p>{isEn ? "The page is live, while signed installers are still in development and security review." : "产品介绍页已经开放，正式签名安装包仍在开发与安全审核中。"}</p></div></div><button className="icon-button" onClick={() => setDesktopReleaseDialog(null)} aria-label={t.close}><X size={19} /></button></header><div className="desktop-release-dialog-body">{desktopReleaseDialog !== "contact" && <><div className="desktop-dialog-status"><span><i />{isEn ? "Core client development in progress" : "核心客户端开发中"}</span><strong>{isEn ? "No public installer yet" : "暂未开放安装包"}</strong></div><div className="desktop-dialog-platforms">{desktopReleaseDialog !== "windows" && <article><AppleLogo size={24} weight="fill" /><div><strong>macOS</strong><small>macOS 13.0+ · Apple Silicon / Intel</small></div><em>{isEn ? "In development" : "开发中"}</em></article>}{desktopReleaseDialog !== "macos" && <article><WindowsLogo size={24} weight="fill" /><div><strong>Windows</strong><small>Windows 10 / 11 · 64-bit</small></div><em>{isEn ? "In development" : "开发中"}</em></article>}</div><p className="desktop-dialog-security"><ShieldCheck size={17} weight="fill" />{isEn ? "For your security, official packages will only be distributed through OneShowTools. Do not install packages shared by third parties." : "为保障账号与数据安全，安装包只会通过 OneShowTools 官方渠道发布，请勿安装第三方转发版本。"}</p></>}<div className="desktop-dialog-contact"><div><span><EnvelopeSimple size={17} /></span><p><small>{isEn ? "Support email" : "联系邮箱"}</small><strong>contact@oneshowailab.com</strong></p><button type="button" onClick={() => copyDesktopContact("contact@oneshowailab.com", isEn ? "Email" : "邮箱")}>{isEn ? "Copy" : "复制"}<Copy size={14} /></button></div><div><span><WechatLogo size={17} /></span><p><small>{isEn ? "WeChat" : "微信号"}</small><strong>OneShowAILab</strong></p><button type="button" onClick={() => copyDesktopContact("OneShowAILab", isEn ? "WeChat ID" : "微信号")}>{isEn ? "Copy" : "复制"}<Copy size={14} /></button></div></div></div><footer className="desktop-release-dialog-footer"><button type="button" className="primary-button" onClick={() => setDesktopReleaseDialog(null)}>{isEn ? "Got it" : "我知道了"}</button></footer></div></div>}
  ...
  </main><aside className={`runtime-side ${runtimeTab === "desktop" ? "desktop-release-side" : ""}`}>{runtimeTab === "desktop" ? <>
    <article className="desktop-release-card surface"><span>{isEn ? "RELEASE PLAN" : "发布计划"}</span><h2>{isEn ? "Built in public, shipped with care" : "稳定优先，分阶段发布"}</h2><ol>{[[CheckCircle, isEn ? "Experience design" : "产品体验设计", isEn ? "Completed" : "已完成", "done"],[SpinnerGap, isEn ? "Core client development" : "核心客户端开发", isEn ? "In progress" : "进行中", "active"],[ShieldCheck, isEn ? "Signing and security review" : "签名与安全审核", isEn ? "Pending" : "待开始", "pending"],[DownloadSimple, isEn ? "Public release" : "正式开放下载", isEn ? "Pending" : "待开始", "pending"]].map(([Icon, title, status, state]) => <li className={state} key={title}><i><Icon size={16} weight={state === "done" ? "fill" : "duotone"} /></i><span><strong>{title}</strong><small>{status}</small></span></li>)}</ol></article>
    <article className="desktop-release-card surface"><span>{isEn ? "FIRST RELEASE" : "首发能力"}</span><h2>{isEn ? "The essentials, ready on day one" : "首个版本重点打磨这些体验"}</h2><ul><li><CheckCircle size={15} weight="fill" />{isEn ? "Shared account and credits" : "账号与积分权益同步"}</li><li><CheckCircle size={15} weight="fill" />{isEn ? "Tool and file access" : "工具与文件中心直达"}</li><li><CheckCircle size={15} weight="fill" />{isEn ? "Background task notifications" : "后台任务状态通知"}</li><li><CheckCircle size={15} weight="fill" />{isEn ? "Secure automatic updates" : "安全的自动更新机制"}</li></ul></article>
    <article className="desktop-release-contact"><DeviceMobile size={22} weight="duotone" /><div><strong>{isEn ? "iPhone and iPad" : "iPhone 与 iPad"}</strong><small>{isEn ? "Mobile clients are on the product roadmap." : "移动端客户端已进入后续产品规划。"}</small></div><a href="mailto:contact@oneshowailab.com">{isEn ? "Ask us" : "联系我们"}<ArrowRight size={13} /></a></article>
  </> : <>
...
function Account({ user, health, credits, billing, locale, onLogout, onUserChange, onLocaleChange, onNotice, onNavigate }) {
  ...
  const copySupportWechat = async () => {
    try {
      await navigator.clipboard.writeText("OneShowAILab");
      onNotice(copy.copiedWechat);
    } catch {
      onNotice(`${copy.wechat}：OneShowAILab`);
    }
  };
...
export function App() {
  ...
  const [view, setView] = useState(() => {
    ...
    return ["dashboard", "marketplace", "recent", "favorites", "agent", "runtime", "downloads", "plans", "tasks", "files", "projects", "settings", "account"].includes(requestedView) ? requestedView : "dashboard";
  });
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem("ost_sidebar_collapsed") === "1");
  ...
  useEffect(() => { localStorage.setItem("ost_sidebar_collapsed", sidebarCollapsed ? "1" : "0"); }, [sidebarCollapsed]);
  useEffect(() => {
  ...
  const navGroups = [
    ...
    { label: locale === "en" ? "DISCOVER" : "发现", items: [["marketplace", SquaresFour], ["recent", Clock], ["favorites", Star]] },
    { label: locale === "en" ? "CREATE" : "创作", items: [["tasks", ListChecks, privateData.tasks.length], ["files", FolderOpen, privateData.files.length], ["projects", Database]] },
    { label: locale === "en" ? "ADVANCED" : "高级", items: [["agent", Robot, "Beta"], ["runtime", RocketLaunch]] },
  ...
  const content = {
    ...
    downloads: <Runtime data={privateData.runtime} dashboard={privateData.dashboard} tasks={privateData.tasks} locale={locale} onRefresh={loadPrivate} onNotice={setToast} onNavigate={setView} initialTab="desktop" />,
  ...
  const usesFullWorkspace = ["dashboard", "marketplace", "recent", "favorites", "agent", "runtime", "downloads", "plans", "projects"].includes(view) || isToolWorkspace;
