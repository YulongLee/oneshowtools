    ...
    <div className={`tier-workspace step-${activeStep}`}>
      ...
      {activeStep === "sort" && <section className="tier-panel tier-ranking tier-ranking-full">
        ...
        <div className={`tier-material-tray ${unassigned.length ? "has-assets" : ""} ${dragOver === "tray" ? "is-drag-over" : ""}`} onMouseEnter={() => dragIdRef.current && setDragOver("tray")} onMouseLeave={() => dragIdRef.current && setDragOver("")} onMouseUp={pointerDropOnTray} onDragEnter={() => dragId && setDragOver("tray")} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setDragOver(""); }} onDragOver={(event) => { if (dragId) { event.preventDefault(); event.dataTransfer.dropEffect = "move"; } }} onDrop={dropOnTray}>
          ...
          <button type="button" className="tier-upload-button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); addFiles(event.dataTransfer.files); }}><CloudArrowUp size={18} />{unassigned.length ? "继续添加图片" : "上传图片"}</button>
        </div>
      ...
      {activeStep === "preview" && <section className="tier-panel tier-preview-panel">
        ...
        <div className={`tier-live-preview theme-${template}`} style={{ aspectRatio: previewRatio }}><header><h3>{title || "夯拉排行榜"}</h3><p>从夯到拉，主观锐评，仅供娱乐</p></header>{tiers.map((tier) => {
          const previewAssets = tier.itemIds.map((id) => assetMap.get(id)).filter(Boolean);
          const previewColumns = Math.max(1, Math.ceil(Math.sqrt(previewAssets.length * 2)));
          return <div className="tier-preview-row" key={tier.id} style={{ "--tier-color": tier.color }}><strong>{tier.name}</strong><span style={{ "--tier-preview-columns": previewColumns }}>{previewAssets.map((asset) => <img key={asset.id} src={asset.url} alt={asset.name} />)}</span></div>;
        })}<footer>你怎么排？评论区见！</footer></div>
